...
> **输入：** 你将收到：
> 1. 页面截图（原型图）
> 2. Agent-1 交接摘要：实体清单、字段定义（含英文字段名）
> 3. Agent-2 交接摘要：业务规则、约束分级
> 4. Agent-3 交接摘要：API 清单、状态机、并发方案、权限矩阵
>
...
2. 附上：截图 + Agent-1 + Agent-2 + Agent-3 的**交接摘要**
3. 首次使用告知 AI 生成基础设施层；追加模块时告知 "基础设施层已存在"
4. 人工审核重点：
   - [ ] DTO 字段是否覆盖原型图所有输入项
