...
{
  "schemaVersion": 1,
  "componentKey": "05:query-history",
  "legacyId": "query-history",
  "category": "05",
...
| 字段 | 值 |
|---|---|
| 旧组件 ID | `query-history` |
| 中文名 / 英文名 | 查询历史 / Query History |
| 类别 / 分组 | 05 查询与筛选类 / 业务复合 |
| 原型来源 | `prototype-suite/catalog.browser.json#05:query-history` |
...
- 摘要：记录查询条件、执行时间与结果规模，支持快速重放。
- 模型：`QueryExecution[] -> RecentQuery[]`
- 原型状态：`entries, pinned, replaying`
...
- 角色：陈敏｜财务经理
- 任务：使用查询历史构造并执行业务查询，核对筛选口径和结果
- 对象：查询历史；订单、库存与应收查询集
- 规则：查询条件、字段口径和数据范围必须可解释且绑定当前租户
- 原型异常：查询历史：查询服务失败、结果为空或条件互相冲突
- 预期影响：查询历史：生成可复用的查询结果并保留条件与执行记录
...
- 载入并核对 查询历史、订单、库存与应收查询集 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
- 通过组件自身控件完成核心操作：记录查询条件、执行时间与结果规模，支持快速重放。
- 按“查询条件、字段口径和数据范围必须可解释且绑定当前租户”校验；校验通过后才产生“查询历史：生成可复用的查询结果并保留条件与执行记录”。
...
- 初始：对象身份、权限、版本和当前 entries, pinned, replaying 可解释。
...
1. 载入并核对 查询历史、订单、库存与应收查询集 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
2. 通过组件自身控件完成核心操作：记录查询条件、执行时间与结果规模，支持快速重放。
3. 按“查询条件、字段口径和数据范围必须可解释且绑定当前租户”校验；校验通过后才产生“查询历史：生成可复用的查询结果并保留条件与执行记录”。

### 4.3 异常与恢复路径

1. 使用可重复夹具真实触发：查询历史：查询服务失败、结果为空或条件互相冲突
...
- 主路径必须实际完成“使用查询历史构造并执行业务查询，核对筛选口径和结果”，不能只改变状态文案或调用统一壳层动作。
- 不变量断言：界面中的条件结构、逻辑分组与最终执行语义必须一致；切换视图不得静默改写查询。
- 异常断言：真实触发“查询历史：查询服务失败、结果为空或条件互相冲突”，并验证业务动作被正确阻断。
- 恢复断言：执行“保留有效条件，移除冲突条件后重试查询”后，输入、对象身份、焦点和可继续操作性符合契约。
- 结果断言：主体区域明确呈现“查询历史：生成可复用的查询结果并保留条件与执行记录”，并能关联到 查询历史、订单、库存与应收查询集。
...
证据目录：`quality/evidence/05-query-history/<component-version>/`
