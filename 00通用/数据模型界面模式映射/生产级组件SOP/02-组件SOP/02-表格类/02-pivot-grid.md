...
|---|---|
| 旧组件 ID | `pivot-grid` |
| 中文名 / 英文名 | 透视表 / Pivot Grid |
| 类别 / 分组 | 02 表格类 / 业务复合 |
| 原型来源 | `prototype-suite/catalog.browser.json#02:pivot-grid` |
...
- 摘要：透视表把结构化数据、运行状态与用户操作组织为可连续理解和操作的界面。
- 模型：`Fact[] × Dimension[] → Cell`
- 原型状态：`rows, columns, measures`
...
- 角色：王蕾｜仓库主管
- 任务：使用透视表处理订单、库存与应收明细并核对结果
- 对象：透视表；销售订单与库存批次 DATA-20260714
- 规则：批量、排序、过滤和编辑必须保持业务主键、版本与汇总口径一致
- 原型异常：透视表：数据加载失败、结果为空或记录版本冲突时禁止静默覆盖
- 预期影响：透视表：更新可追溯的数据视图并保留批次与版本审计
...
- 载入并核对 透视表、销售订单与库存批次 DATA-20260714 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
- 通过组件自身控件完成核心操作：透视表把结构化数据、运行状态与用户操作组织为可连续理解和操作的界面。
- 按“批量、排序、过滤和编辑必须保持业务主键、版本与汇总口径一致”校验；校验通过后才产生“透视表：更新可追溯的数据视图并保留批次与版本审计”。
...
- 初始：对象身份、权限、版本和当前 rows, columns, measures 可解释。
...
1. 载入并核对 透视表、销售订单与库存批次 DATA-20260714 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
2. 通过组件自身控件完成核心操作：透视表把结构化数据、运行状态与用户操作组织为可连续理解和操作的界面。
3. 按“批量、排序、过滤和编辑必须保持业务主键、版本与汇总口径一致”校验；校验通过后才产生“透视表：更新可追溯的数据视图并保留批次与版本审计”。

### 4.3 异常与恢复路径

1. 使用可重复夹具真实触发：透视表：数据加载失败、结果为空或记录版本冲突时禁止静默覆盖
...
- 主路径必须实际完成“使用透视表处理订单、库存与应收明细并核对结果”，不能只改变状态文案或调用统一壳层动作。
- 不变量断言：对象身份、层级或字段路径必须稳定；筛选、切换与编辑不能静默丢失用户状态。
- 异常断言：真实触发“透视表：数据加载失败、结果为空或记录版本冲突时禁止静默覆盖”，并验证业务动作被正确阻断。
- 恢复断言：执行“保留当前条件并重新加载最新数据版本”后，输入、对象身份、焦点和可继续操作性符合契约。
- 结果断言：主体区域明确呈现“透视表：更新可追溯的数据视图并保留批次与版本审计”，并能关联到 透视表、销售订单与库存批次 DATA-20260714。
...
证据目录：`quality/evidence/02-pivot-grid/<component-version>/`
