...
{
  "schemaVersion": 1,
  "componentKey": "09:concept-map",
  "legacyId": "concept-map",
  "category": "09",
...
|---|---|
| 旧组件 ID | `concept-map` |
| 中文名 / 英文名 | 概念关系图 / Concept Map |
| 类别 / 分组 | 09 流程、规则与图形编排类 / 业务复合 |
| 原型来源 | `prototype-suite/catalog.browser.json#09:concept-map` |
...
- 摘要：概念关系图允许多对多连接并为边标注语义，表达非层级知识结构。
- 模型：`Concept[] + TypedRelation[]`
- 原型状态：`selectedConcept, relationType, zoom`
- 不变量：连接词能形成可读命题，概念身份稳定。
- 适用边界：适合概念学习和领域建模；严格数据结构使用 ER 或类图。

### 2.2 业务上下文

- 角色：周宁｜销售经理
- 任务：使用概念关系图编排并验证流程、规则与图形关系
- 对象：概念关系图；销售审批、信用与库存调拨流程
- 规则：连接、条件和状态转换必须合法，循环和冲突规则必须在发布前阻断
- 原型异常：概念关系图：非法连接、规则冲突、循环依赖或流程发布失败
- 预期影响：概念关系图：生成可执行的流程版本、影响说明和审计记录
...
- 载入并核对 概念关系图、销售审批、信用与库存调拨流程 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
- 通过组件自身控件完成核心操作：概念关系图允许多对多连接并为边标注语义，表达非层级知识结构。
- 按“连接、条件和状态转换必须合法，循环和冲突规则必须在发布前阻断”校验；校验通过后才产生“概念关系图：生成可执行的流程版本、影响说明和审计记录”。
...
- 初始：对象身份、权限、版本和当前 selectedConcept, relationType, zoom 可解释。
...
1. 载入并核对 概念关系图、销售审批、信用与库存调拨流程 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
2. 通过组件自身控件完成核心操作：概念关系图允许多对多连接并为边标注语义，表达非层级知识结构。
3. 按“连接、条件和状态转换必须合法，循环和冲突规则必须在发布前阻断”校验；校验通过后才产生“概念关系图：生成可执行的流程版本、影响说明和审计记录”。

### 4.3 异常与恢复路径

1. 使用可重复夹具真实触发：概念关系图：非法连接、规则冲突、循环依赖或流程发布失败
...
- 主路径必须实际完成“使用概念关系图编排并验证流程、规则与图形关系”，不能只改变状态文案或调用统一壳层动作。
- 不变量断言：连接词能形成可读命题，概念身份稳定。
- 异常断言：真实触发“概念关系图：非法连接、规则冲突、循环依赖或流程发布失败”，并验证业务动作被正确阻断。
- 恢复断言：执行“撤销非法变更，定位冲突节点并恢复最近有效流程版本”后，输入、对象身份、焦点和可继续操作性符合契约。
- 结果断言：主体区域明确呈现“概念关系图：生成可执行的流程版本、影响说明和审计记录”，并能关联到 概念关系图、销售审批、信用与库存调拨流程。
- 边界断言：适合概念学习和领域建模；严格数据结构使用 ER 或类图。
...
证据目录：`quality/evidence/09-concept-map/<component-version>/`
