...
|---|---|
| 旧组件 ID | `masonry` |
| 中文名 / 英文名 | 瀑布流布局 / Masonry |
| 类别 / 分组 | 01 布局类 / 容器组织 |
| 原型来源 | `prototype-suite/catalog.browser.json#01:masonry` |
...
- 摘要：把不同高度的内容按列紧密堆叠，减少垂直空洞并保持视觉浏览节奏。
- 模型：`Masonry → Column[] → VariableItem`
- 原型状态：`columns, itemHeight`
- 不变量：视觉顺序不能破坏业务排序，新增内容不得导致明显跳动。
- 适用边界：适合图片、素材和内容发现；严格行对齐与键盘顺序场景不适用。

### 2.2 业务上下文

- 角色：周宁｜销售经理
- 任务：使用瀑布流布局组织订单处理信息并保持岗位工作上下文
- 对象：瀑布流布局；销售订单 SO-20260714-018 工作区
- 规则：关键订单信息必须可达，布局变化不得改变阅读顺序或丢失未保存上下文
- 原型异常：瀑布流布局：窄屏、面板收起或空间不足导致关键订单操作不可达
- 预期影响：瀑布流布局：保留订单上下文并恢复可操作的工作区布局
...
- 载入并核对 瀑布流布局、销售订单 SO-20260714-018 工作区 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
- 通过组件自身控件完成核心操作：改变列数并新增不同高度的内容块。
- 按“关键订单信息必须可达，布局变化不得改变阅读顺序或丢失未保存上下文”校验；校验通过后才产生“瀑布流布局：保留订单上下文并恢复可操作的工作区布局”。
...
- 初始：对象身份、权限、版本和当前 columns, itemHeight 可解释。
...
1. 载入并核对 瀑布流布局、销售订单 SO-20260714-018 工作区 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
2. 通过组件自身控件完成核心操作：改变列数并新增不同高度的内容块。
3. 按“关键订单信息必须可达，布局变化不得改变阅读顺序或丢失未保存上下文”校验；校验通过后才产生“瀑布流布局：保留订单上下文并恢复可操作的工作区布局”。

### 4.3 异常与恢复路径

1. 使用可重复夹具真实触发：瀑布流布局：窄屏、面板收起或空间不足导致关键订单操作不可达
...
- 主路径必须实际完成“使用瀑布流布局组织订单处理信息并保持岗位工作上下文”，不能只改变状态文案或调用统一壳层动作。
- 不变量断言：视觉顺序不能破坏业务排序，新增内容不得导致明显跳动。
- 异常断言：真实触发“瀑布流布局：窄屏、面板收起或空间不足导致关键订单操作不可达”，并验证业务动作被正确阻断。
- 恢复断言：执行“恢复上一个可用布局并把焦点送回触发控件”后，输入、对象身份、焦点和可继续操作性符合契约。
- 结果断言：主体区域明确呈现“瀑布流布局：保留订单上下文并恢复可操作的工作区布局”，并能关联到 瀑布流布局、销售订单 SO-20260714-018 工作区。
- 边界断言：适合图片、素材和内容发现；严格行对齐与键盘顺序场景不适用。
...
证据目录：`quality/evidence/01-masonry/<component-version>/`
