...
{
  "schemaVersion": 1,
  "componentKey": "01:free-canvas",
  "legacyId": "free-canvas",
  "category": "01",
...
|---|---|
| 旧组件 ID | `free-canvas` |
| 中文名 / 英文名 | 自由画布布局 / Free Canvas |
| 类别 / 分组 | 01 布局类 / 自由编排 |
| 原型来源 | `prototype-suite/catalog.browser.json#01:free-canvas` |
...
- 摘要：在连续坐标空间中摆放节点，支持平移、缩放和自由位置关系。
- 模型：`Canvas → Node[] + ViewportTransform`
- 原型状态：`x, y, zoom, selection`
- 不变量：坐标变换可逆，节点在缩放和平移后仍可选中与恢复。
- 适用边界：适合流程图、白板和拓扑；普通业务页面不应依赖绝对坐标。

### 2.2 业务上下文

- 角色：周宁｜销售经理
- 任务：使用自由画布布局组织订单处理信息并保持岗位工作上下文
- 对象：自由画布布局；销售订单 SO-20260714-018 工作区
- 规则：关键订单信息必须可达，布局变化不得改变阅读顺序或丢失未保存上下文
- 原型异常：自由画布布局：窄屏、面板收起或空间不足导致关键订单操作不可达
- 预期影响：自由画布布局：保留订单上下文并恢复可操作的工作区布局
...
- 载入并核对 自由画布布局、销售订单 SO-20260714-018 工作区 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
- 通过组件自身控件完成核心操作：拖动节点或调整缩放，体验连续坐标空间。
- 按“关键订单信息必须可达，布局变化不得改变阅读顺序或丢失未保存上下文”校验；校验通过后才产生“自由画布布局：保留订单上下文并恢复可操作的工作区布局”。
...
- 初始：对象身份、权限、版本和当前 x, y, zoom, selection 可解释。
...
1. 载入并核对 自由画布布局、销售订单 SO-20260714-018 工作区 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
2. 通过组件自身控件完成核心操作：拖动节点或调整缩放，体验连续坐标空间。
3. 按“关键订单信息必须可达，布局变化不得改变阅读顺序或丢失未保存上下文”校验；校验通过后才产生“自由画布布局：保留订单上下文并恢复可操作的工作区布局”。

### 4.3 异常与恢复路径

1. 使用可重复夹具真实触发：自由画布布局：窄屏、面板收起或空间不足导致关键订单操作不可达
...
- 主路径必须实际完成“使用自由画布布局组织订单处理信息并保持岗位工作上下文”，不能只改变状态文案或调用统一壳层动作。
- 不变量断言：坐标变换可逆，节点在缩放和平移后仍可选中与恢复。
- 异常断言：真实触发“自由画布布局：窄屏、面板收起或空间不足导致关键订单操作不可达”，并验证业务动作被正确阻断。
- 恢复断言：执行“恢复上一个可用布局并把焦点送回触发控件”后，输入、对象身份、焦点和可继续操作性符合契约。
- 结果断言：主体区域明确呈现“自由画布布局：保留订单上下文并恢复可操作的工作区布局”，并能关联到 自由画布布局、销售订单 SO-20260714-018 工作区。
- 边界断言：适合流程图、白板和拓扑；普通业务页面不应依赖绝对坐标。
...
证据目录：`quality/evidence/01-free-canvas/<component-version>/`
