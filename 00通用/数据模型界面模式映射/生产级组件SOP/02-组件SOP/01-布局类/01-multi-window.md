...
{
  "schemaVersion": 1,
  "componentKey": "01:multi-window",
  "legacyId": "multi-window",
  "category": "01",
...
|---|---|
| 旧组件 ID | `multi-window` |
| 中文名 / 英文名 | 多窗口工作区 / Multi-window Workspace |
| 类别 / 分组 | 01 布局类 / 工作区管理 |
| 原型来源 | `prototype-suite/catalog.browser.json#01:multi-window` |
...
- 摘要：在一个工作区内管理多个可叠放窗口，通过位置和层级支持并行任务。
- 模型：`Desktop → Window[] → Rect + ZIndex`
- 原型状态：`x, y, z, active`
- 不变量：活动窗口明确，窗口标题栏始终能被找回和操作。
- 适用边界：适合专业桌面式工具；触屏与普通后台页面应谨慎使用。

### 2.2 业务上下文

- 角色：周宁｜销售经理
- 任务：使用多窗口工作区组织订单处理信息并保持岗位工作上下文
- 对象：多窗口工作区；销售订单 SO-20260714-018 工作区
- 规则：关键订单信息必须可达，布局变化不得改变阅读顺序或丢失未保存上下文
- 原型异常：多窗口工作区：窄屏、面板收起或空间不足导致关键订单操作不可达
- 预期影响：多窗口工作区：保留订单上下文并恢复可操作的工作区布局
...
- 载入并核对 多窗口工作区、销售订单 SO-20260714-018 工作区 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
- 通过组件自身控件完成核心操作：拖动窗口标题栏，点击窗口改变前后层级。
- 按“关键订单信息必须可达，布局变化不得改变阅读顺序或丢失未保存上下文”校验；校验通过后才产生“多窗口工作区：保留订单上下文并恢复可操作的工作区布局”。
...
- 初始：对象身份、权限、版本和当前 x, y, z, active 可解释。
...
1. 载入并核对 多窗口工作区、销售订单 SO-20260714-018 工作区 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
2. 通过组件自身控件完成核心操作：拖动窗口标题栏，点击窗口改变前后层级。
3. 按“关键订单信息必须可达，布局变化不得改变阅读顺序或丢失未保存上下文”校验；校验通过后才产生“多窗口工作区：保留订单上下文并恢复可操作的工作区布局”。

### 4.3 异常与恢复路径

1. 使用可重复夹具真实触发：多窗口工作区：窄屏、面板收起或空间不足导致关键订单操作不可达
...
- 主路径必须实际完成“使用多窗口工作区组织订单处理信息并保持岗位工作上下文”，不能只改变状态文案或调用统一壳层动作。
- 不变量断言：活动窗口明确，窗口标题栏始终能被找回和操作。
- 异常断言：真实触发“多窗口工作区：窄屏、面板收起或空间不足导致关键订单操作不可达”，并验证业务动作被正确阻断。
- 恢复断言：执行“恢复上一个可用布局并把焦点送回触发控件”后，输入、对象身份、焦点和可继续操作性符合契约。
- 结果断言：主体区域明确呈现“多窗口工作区：保留订单上下文并恢复可操作的工作区布局”，并能关联到 多窗口工作区、销售订单 SO-20260714-018 工作区。
- 边界断言：适合专业桌面式工具；触屏与普通后台页面应谨慎使用。
...
证据目录：`quality/evidence/01-multi-window/<component-version>/`
