...
- **生产交付总 SOP 定义公共过程**：登记、评审、RED/GREEN、风险加固、发布、观察和回滚。
- **组件实施 SOP 定义具体作业过程**：根据某个组件规范给出实现顺序、失败注入、组合测试、证据和停止条件。
...
- `ReviewReady`：5 个金标准候选，公开 API 仍为 `proposed`；部分存在未决项，全部等待规定角色审批。
...
├─ 03-生产SOP/             # 1 份生产交付总 SOP + 按独立组件编写的实施 SOP
...
4. 读取[生产 SOP 分层与使用规则](03-生产SOP/README.md)，同时执行生产交付总 SOP 和该组件对应的实施 SOP。
5. 通过准入后才从 RED 开始生产实现；缺少组件实施 SOP 时不得编码。
6. 按[证据规范](05-证据规范/README.md)保存与规范、两层 SOP、源码和不可变产物绑定的证据。

表格组件族已经选择“DataGrid 核心 + 内部能力模块 + TanStack 适配层”的演进路线，34 项目录能力的归类、首批范围与 TreeGrid 扩展边界见[表格组件族实施设计](02-组件规范/02-表格类/02-表格组件族实施设计.md)。DataGrid 的具体作业步骤见[DataGrid 组件实施 SOP](03-生产SOP/组件实施SOP/02-data-grid.implementation-sop.md)。技术设计和实施 SOP 都不能绕过组件规范准入。
