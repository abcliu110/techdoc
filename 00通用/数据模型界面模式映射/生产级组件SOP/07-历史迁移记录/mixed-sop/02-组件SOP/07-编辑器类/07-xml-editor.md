...
|---|---|
| 旧组件 ID | `xml-editor` |
| 中文名 / 英文名 | XML 编辑器 / XML Editor |
| 类别 / 分组 | 07 编辑器类 / 业务复合 |
| 原型来源 | `prototype-suite/catalog.browser.json#07:xml-editor` |
...
- 摘要：在标签文本与元素树之间同步，支持命名空间和 Schema 校验。
- 模型：`XML Text <-> ElementTree`
- 原型状态：`namespaces, path, validation`
...
- 角色：李明｜实施顾问
- 任务：使用XML 编辑器编辑、校验并预览业务内容
- 对象：XML 编辑器；合同、规则、SQL 与设备配置草稿
- 规则：解析、引用和发布前校验必须通过，未保存版本不得静默丢失
- 原型异常：XML 编辑器：语法解析失败、引用失效或内容版本发生冲突
- 预期影响：XML 编辑器：形成可预览、可回滚且带版本记录的编辑结果
...
- 载入并核对 XML 编辑器、合同、规则、SQL 与设备配置草稿 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
- 通过组件自身控件完成核心操作：在标签文本与元素树之间同步，支持命名空间和 Schema 校验。
- 按“解析、引用和发布前校验必须通过，未保存版本不得静默丢失”校验；校验通过后才产生“XML 编辑器：形成可预览、可回滚且带版本记录的编辑结果”。
...
- 初始：对象身份、权限、版本和当前 namespaces, path, validation 可解释。
...
1. 载入并核对 XML 编辑器、合同、规则、SQL 与设备配置草稿 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
2. 通过组件自身控件完成核心操作：在标签文本与元素树之间同步，支持命名空间和 Schema 校验。
3. 按“解析、引用和发布前校验必须通过，未保存版本不得静默丢失”校验；校验通过后才产生“XML 编辑器：形成可预览、可回滚且带版本记录的编辑结果”。

### 4.3 异常与恢复路径

1. 使用可重复夹具真实触发：XML 编辑器：语法解析失败、引用失效或内容版本发生冲突
...
- 主路径必须实际完成“使用XML 编辑器编辑、校验并预览业务内容”，不能只改变状态文案或调用统一壳层动作。
- 不变量断言：编辑源、可视化投影与持久化结果必须可逆且语义一致；校验失败时不得覆盖上一个有效版本。
- 异常断言：真实触发“XML 编辑器：语法解析失败、引用失效或内容版本发生冲突”，并验证业务动作被正确阻断。
- 恢复断言：执行“定位错误并恢复最近有效草稿或撤销最后一次编辑”后，输入、对象身份、焦点和可继续操作性符合契约。
- 结果断言：主体区域明确呈现“XML 编辑器：形成可预览、可回滚且带版本记录的编辑结果”，并能关联到 XML 编辑器、合同、规则、SQL 与设备配置草稿。
...
证据目录：`quality/evidence/07-xml-editor/<component-version>/`
