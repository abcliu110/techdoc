...
{
  "schemaVersion": 1,
  "componentKey": "05:saved-query",
  "legacyId": "saved-query",
  "category": "05",
...
|---|---|
| 旧组件 ID | `saved-query` |
| 中文名 / 英文名 | 保存查询 / Saved Query |
| 类别 / 分组 | 05 查询与筛选类 / 业务复合 |
| 原型来源 | `prototype-suite/catalog.browser.json#05:saved-query` |
...
- 摘要：将稳定的查询结构命名保存，用于复用、共享和版本管理。
- 模型：`SavedQuery -> QuerySnapshot`
- 原型状态：`savedItems, activeId, dirty`
...
- 角色：陈敏｜财务经理
- 任务：使用保存查询构造并执行业务查询，核对筛选口径和结果
- 对象：保存查询；订单、库存与应收查询集
- 规则：查询条件、字段口径和数据范围必须可解释且绑定当前租户
- 原型异常：保存查询：查询服务失败、结果为空或条件互相冲突
- 预期影响：保存查询：生成可复用的查询结果并保留条件与执行记录
...
- 载入并核对 保存查询、订单、库存与应收查询集 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
- 通过组件自身控件完成核心操作：将稳定的查询结构命名保存，用于复用、共享和版本管理。
- 按“查询条件、字段口径和数据范围必须可解释且绑定当前租户”校验；校验通过后才产生“保存查询：生成可复用的查询结果并保留条件与执行记录”。
...
- 初始：对象身份、权限、版本和当前 savedItems, activeId, dirty 可解释。
...
1. 载入并核对 保存查询、订单、库存与应收查询集 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
2. 通过组件自身控件完成核心操作：将稳定的查询结构命名保存，用于复用、共享和版本管理。
3. 按“查询条件、字段口径和数据范围必须可解释且绑定当前租户”校验；校验通过后才产生“保存查询：生成可复用的查询结果并保留条件与执行记录”。

### 4.3 异常与恢复路径

1. 使用可重复夹具真实触发：保存查询：查询服务失败、结果为空或条件互相冲突
...
- 主路径必须实际完成“使用保存查询构造并执行业务查询，核对筛选口径和结果”，不能只改变状态文案或调用统一壳层动作。
- 不变量断言：界面中的条件结构、逻辑分组与最终执行语义必须一致；切换视图不得静默改写查询。
- 异常断言：真实触发“保存查询：查询服务失败、结果为空或条件互相冲突”，并验证业务动作被正确阻断。
- 恢复断言：执行“保留有效条件，移除冲突条件后重试查询”后，输入、对象身份、焦点和可继续操作性符合契约。
- 结果断言：主体区域明确呈现“保存查询：生成可复用的查询结果并保留条件与执行记录”，并能关联到 保存查询、订单、库存与应收查询集。
...
证据目录：`quality/evidence/05-saved-query/<component-version>/`
