...
{
  "schemaVersion": 1,
  "componentKey": "01:grid-layout",
  "legacyId": "grid-layout",
  "category": "01",
...
|---|---|
| 旧组件 ID | `grid-layout` |
| 中文名 / 英文名 | 栅格布局 / Grid Layout |
| 类别 / 分组 | 01 布局类 / 空间划分 |
| 原型来源 | `prototype-suite/catalog.browser.json#01:grid-layout` |
...
- 摘要：把可用空间离散为行、列和间距，再按跨行跨列规则放置内容。
- 模型：`Grid → Track → Area → Item`
- 原型状态：`columns, gap, span`
- 不变量：轨道总宽度不得突破容器；断点切换后内容仍可达。
- 适用边界：适合规则网格、仪表盘和表单区域；不适合自由坐标画布。

### 2.2 业务上下文

- 角色：周宁｜销售经理
- 任务：使用栅格布局组织订单处理信息并保持岗位工作上下文
- 对象：栅格布局；销售订单 SO-20260714-018 工作区
- 规则：关键订单信息必须可达，布局变化不得改变阅读顺序或丢失未保存上下文
- 原型异常：栅格布局：窄屏、面板收起或空间不足导致关键订单操作不可达
- 预期影响：栅格布局：保留订单上下文并恢复可操作的工作区布局
...
- 载入并核对 栅格布局、销售订单 SO-20260714-018 工作区 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
- 通过组件自身控件完成核心操作：调整列数，观察跨列项如何重新排布。
- 按“关键订单信息必须可达，布局变化不得改变阅读顺序或丢失未保存上下文”校验；校验通过后才产生“栅格布局：保留订单上下文并恢复可操作的工作区布局”。
...
- 初始：对象身份、权限、版本和当前 columns, gap, span 可解释。
...
1. 载入并核对 栅格布局、销售订单 SO-20260714-018 工作区 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
2. 通过组件自身控件完成核心操作：调整列数，观察跨列项如何重新排布。
3. 按“关键订单信息必须可达，布局变化不得改变阅读顺序或丢失未保存上下文”校验；校验通过后才产生“栅格布局：保留订单上下文并恢复可操作的工作区布局”。

### 4.3 异常与恢复路径

1. 使用可重复夹具真实触发：栅格布局：窄屏、面板收起或空间不足导致关键订单操作不可达
...
- 主路径必须实际完成“使用栅格布局组织订单处理信息并保持岗位工作上下文”，不能只改变状态文案或调用统一壳层动作。
- 不变量断言：轨道总宽度不得突破容器；断点切换后内容仍可达。
- 异常断言：真实触发“栅格布局：窄屏、面板收起或空间不足导致关键订单操作不可达”，并验证业务动作被正确阻断。
- 恢复断言：执行“恢复上一个可用布局并把焦点送回触发控件”后，输入、对象身份、焦点和可继续操作性符合契约。
- 结果断言：主体区域明确呈现“栅格布局：保留订单上下文并恢复可操作的工作区布局”，并能关联到 栅格布局、销售订单 SO-20260714-018 工作区。
- 边界断言：适合规则网格、仪表盘和表单区域；不适合自由坐标画布。
...
证据目录：`quality/evidence/01-grid-layout/<component-version>/`
