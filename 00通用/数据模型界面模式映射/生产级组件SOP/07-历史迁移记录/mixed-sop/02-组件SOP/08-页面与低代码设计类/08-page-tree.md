...
|---|---|
| 旧组件 ID | `page-tree` |
| 中文名 / 英文名 | 页面结构树 / Page Structure Tree |
| 类别 / 分组 | 08 页面与低代码设计类 / 业务复合 |
| 原型来源 | `prototype-suite/catalog.browser.json#08:page-tree` |
...
- 摘要：页面结构树用层级方式呈现容器与组件的父子关系，支持选择、重排和定位。
- 模型：`PageNode → ChildNode[]`
- 原型状态：`expandedIds, selectedId, order`
- 不变量：树顺序与画布层级同步，任何节点始终只有一个父容器。
- 适用边界：适合嵌套页面；深度过大时应提示结构复杂度。

### 2.2 业务上下文

- 角色：李明｜实施顾问
- 任务：使用页面结构树配置应用结构、数据源、事件并完成预览或发布
- 对象：页面结构树；订单应用与工单应用版本
- 规则：发布前依赖、权限、数据源和版本差异必须全部通过校验
- 原型异常：页面结构树：组件依赖缺失、数据源不可用或发布版本冲突
- 预期影响：页面结构树：生成可审计的应用版本并通知受影响岗位
...
- 载入并核对 页面结构树、订单应用与工单应用版本 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
- 通过组件自身控件完成核心操作：页面结构树用层级方式呈现容器与组件的父子关系，支持选择、重排和定位。
- 按“发布前依赖、权限、数据源和版本差异必须全部通过校验”校验；校验通过后才产生“页面结构树：生成可审计的应用版本并通知受影响岗位”。
...
- 初始：对象身份、权限、版本和当前 expandedIds, selectedId, order 可解释。
...
1. 载入并核对 页面结构树、订单应用与工单应用版本 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
2. 通过组件自身控件完成核心操作：页面结构树用层级方式呈现容器与组件的父子关系，支持选择、重排和定位。
3. 按“发布前依赖、权限、数据源和版本差异必须全部通过校验”校验；校验通过后才产生“页面结构树：生成可审计的应用版本并通知受影响岗位”。

### 4.3 异常与恢复路径

1. 使用可重复夹具真实触发：页面结构树：组件依赖缺失、数据源不可用或发布版本冲突
...
- 主路径必须实际完成“使用页面结构树配置应用结构、数据源、事件并完成预览或发布”，不能只改变状态文案或调用统一壳层动作。
- 不变量断言：树顺序与画布层级同步，任何节点始终只有一个父容器。
- 异常断言：真实触发“页面结构树：组件依赖缺失、数据源不可用或发布版本冲突”，并验证业务动作被正确阻断。
- 恢复断言：执行“保留当前设计，修复校验项或回滚到最近已发布版本”后，输入、对象身份、焦点和可继续操作性符合契约。
- 结果断言：主体区域明确呈现“页面结构树：生成可审计的应用版本并通知受影响岗位”，并能关联到 页面结构树、订单应用与工单应用版本。
- 边界断言：适合嵌套页面；深度过大时应提示结构复杂度。
...
证据目录：`quality/evidence/08-page-tree/<component-version>/`
