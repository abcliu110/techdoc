...
|---|---|
| 旧组件 ID | `cascader` |
| 中文名 / 英文名 | 级联选择器 / Cascader |
| 类别 / 分组 | 06 选择器类 / 业务复合 |
| 原型来源 | `prototype-suite/catalog.browser.json#06:cascader` |
...
- 摘要：通过逐级列表完成路径选择，适合层级稳定的分类数据。
- 模型：`Hierarchy -> PathKey[]`
- 原型状态：`activePath, selectedPath`
...
- 角色：林晓｜销售代表
- 任务：使用级联选择器在业务范围内定位并确认关联对象
- 对象：级联选择器；客户、商品、人员与组织主数据
- 规则：候选项必须满足租户、组织、有效期和数据权限范围
- 原型异常：级联选择器：远程加载失败、无匹配结果或候选对象无访问权限
- 预期影响：级联选择器：建立可解释的业务关联并保留来源上下文
...
- 载入并核对 级联选择器、客户、商品、人员与组织主数据 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
- 通过组件自身控件完成核心操作：通过逐级列表完成路径选择，适合层级稳定的分类数据。
- 按“候选项必须满足租户、组织、有效期和数据权限范围”校验；校验通过后才产生“级联选择器：建立可解释的业务关联并保留来源上下文”。
...
- 初始：对象身份、权限、版本和当前 activePath, selectedPath 可解释。
...
1. 载入并核对 级联选择器、客户、商品、人员与组织主数据 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
2. 通过组件自身控件完成核心操作：通过逐级列表完成路径选择，适合层级稳定的分类数据。
3. 按“候选项必须满足租户、组织、有效期和数据权限范围”校验；校验通过后才产生“级联选择器：建立可解释的业务关联并保留来源上下文”。

### 4.3 异常与恢复路径

1. 使用可重复夹具真实触发：级联选择器：远程加载失败、无匹配结果或候选对象无访问权限
...
- 主路径必须实际完成“使用级联选择器在业务范围内定位并确认关联对象”，不能只改变状态文案或调用统一壳层动作。
- 不变量断言：已选值的稳定标识、显示标签与来源对象必须一致；过滤、分页和懒加载不得丢失已选项。
- 异常断言：真实触发“级联选择器：远程加载失败、无匹配结果或候选对象无访问权限”，并验证业务动作被正确阻断。
- 恢复断言：执行“清除无效选择，恢复搜索条件并重新加载候选项”后，输入、对象身份、焦点和可继续操作性符合契约。
- 结果断言：主体区域明确呈现“级联选择器：建立可解释的业务关联并保留来源上下文”，并能关联到 级联选择器、客户、商品、人员与组织主数据。
...
证据目录：`quality/evidence/06-cascader/<component-version>/`
