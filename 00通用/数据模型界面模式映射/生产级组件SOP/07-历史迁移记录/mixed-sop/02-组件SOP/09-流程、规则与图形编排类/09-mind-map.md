...
|---|---|
| 旧组件 ID | `mind-map` |
| 中文名 / 英文名 | 思维导图编辑器 / Mind Map Editor |
| 类别 / 分组 | 09 流程、规则与图形编排类 / 业务复合 |
| 原型来源 | `prototype-suite/catalog.browser.json#09:mind-map` |
...
- 摘要：思维导图从中心主题辐射分支，支持快速发散、归类和收敛。
- 模型：`TopicNode → ChildTopic[]`
- 原型状态：`focusTopic, collapsedIds, zoom`
- 不变量：每个主题有唯一父级，折叠不丢失子主题。
- 适用边界：适合主题分解；精确流程和多对多关系使用对应图形。

### 2.2 业务上下文

- 角色：周宁｜销售经理
- 任务：使用思维导图编辑器编排并验证流程、规则与图形关系
- 对象：思维导图编辑器；销售审批、信用与库存调拨流程
- 规则：连接、条件和状态转换必须合法，循环和冲突规则必须在发布前阻断
- 原型异常：思维导图编辑器：非法连接、规则冲突、循环依赖或流程发布失败
- 预期影响：思维导图编辑器：生成可执行的流程版本、影响说明和审计记录
...
- 载入并核对 思维导图编辑器、销售审批、信用与库存调拨流程 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
- 通过组件自身控件完成核心操作：思维导图从中心主题辐射分支，支持快速发散、归类和收敛。
- 按“连接、条件和状态转换必须合法，循环和冲突规则必须在发布前阻断”校验；校验通过后才产生“思维导图编辑器：生成可执行的流程版本、影响说明和审计记录”。
...
- 初始：对象身份、权限、版本和当前 focusTopic, collapsedIds, zoom 可解释。
...
1. 载入并核对 思维导图编辑器、销售审批、信用与库存调拨流程 的身份、版本和当前状态；此动作属于组件业务前置，不得用统一“开始”按钮代替。
2. 通过组件自身控件完成核心操作：思维导图从中心主题辐射分支，支持快速发散、归类和收敛。
3. 按“连接、条件和状态转换必须合法，循环和冲突规则必须在发布前阻断”校验；校验通过后才产生“思维导图编辑器：生成可执行的流程版本、影响说明和审计记录”。

### 4.3 异常与恢复路径

1. 使用可重复夹具真实触发：思维导图编辑器：非法连接、规则冲突、循环依赖或流程发布失败
...
- 主路径必须实际完成“使用思维导图编辑器编排并验证流程、规则与图形关系”，不能只改变状态文案或调用统一壳层动作。
- 不变量断言：每个主题有唯一父级，折叠不丢失子主题。
- 异常断言：真实触发“思维导图编辑器：非法连接、规则冲突、循环依赖或流程发布失败”，并验证业务动作被正确阻断。
- 恢复断言：执行“撤销非法变更，定位冲突节点并恢复最近有效流程版本”后，输入、对象身份、焦点和可继续操作性符合契约。
- 结果断言：主体区域明确呈现“思维导图编辑器：生成可执行的流程版本、影响说明和审计记录”，并能关联到 思维导图编辑器、销售审批、信用与库存调拨流程。
- 边界断言：适合主题分解；精确流程和多对多关系使用对应图形。
...
证据目录：`quality/evidence/09-mind-map/<component-version>/`
