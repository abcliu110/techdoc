...
画布节点（canvas-node）的点击事件需要区分交互元素点击和选择区域点击：

| 点击目标 | 行为 |
|---|---|
| 交互元素（input/textarea/select/button） | 事件正常传播，不触发节点选中 |
| 控制栏（拖拽手柄、删除按钮） | 触发对应操作（拖拽、删除），停止传播 |
| 其他节点内容区域 | 触发节点选中（高亮边框 + 属性面板显示） |
| 画布空白区域 | 取消当前选中 |

实现方式：使用透明选择区域叠加层（z-index: 5）覆盖节点内容区，仅当点击非交互元素时触发 `onSelect`。

#### 8.2 控制栏可见性策略

节点控制栏（拖拽手柄、删除按钮）可见性规则：

| 状态 | opacity | 说明 |
|---|---|---|
| 节点未选中 | `0.6` | 半透明可见，提示存在操作能力 |
| 节点选中 | `1.0` | 完全可见 |
| 鼠标悬停（控制栏区域） | `1.0` | 悬停时完全可见 |

默认值从 `0`（完全隐藏）改为 `0.6`（半透明），降低用户认知负担，无需悬停即可看到操作提示。

#### 8.3 容器组件子组件规范

布局容器组件（card、tabs、collapse）支持任意数量子组件嵌套：

**子组件操作：**
- **添加**：从左侧组件面板拖入容器 drop-zone，或点击添加按钮
- **删除**：选中子组件后点击删除按钮，支持任意嵌套深度递归删除
- **排序**：拖拽调整子组件顺序
- **嵌套**：子组件本身也可以是容器组件，支持多层嵌套

**数据模型：**
```typescript
interface ContainerField extends DesignerFieldData {
  children?: DesignerFieldData[];  // 递归嵌套子组件
...
**拖拽交互四种情况：**
1. 组件面板 → 画布根级：直接添加为根级字段
2. 组件面板 → 容器 drop-zone：递归查找目标容器并添加子组件
3. 画布字段 → 容器 drop-zone：将字段从原位置移动到目标容器
4. 画布字段 → 画布字段：根级字段排序
...
| 组件类别 | 组件示例 | 默认宽度 | 默认高度 |
|---|---|---|---|
| 行内/自适应 | button、checkbox、radio、switch、rate、tag、divider、colorPicker | `auto` | 无 |
| 表单输入类 | input、inputNumber、textarea、select、cascader、tree、autoComplete、datePicker、timePicker、rangePicker、upload、slider、transfer、richText、subTable、calendar | `240px` | 无 |
| 布局容器类 | card、tabs、collapse | `100%` | 无 |

尺寸单位默认为 `px`，支持切换为 `%`、`rem`、`em`、`vw`、`vh`、`auto`。
