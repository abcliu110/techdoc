...
class RuntimeApiHttpFacade implements RuntimeHttpFacade {
  ...
  RuntimeApiHttpFacade(
    ...
    if (runtimeDemoEnabled && objectRegistry == null && runtimeApiFacade.isPresent()) {
      registerM1DemoObjects(runtimeApiFacade.get());
    }
  }
  ...
  private RuntimeApiFacade runtimeApiFacade() {
    return runtimeApiFacade.orElseThrow(() -> new BizException(ErrorCode.FEATURE_DISABLED, "运行时服务未启用"));
  }
