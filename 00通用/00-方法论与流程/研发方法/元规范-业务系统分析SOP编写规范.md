...
> **版本**：v3.2 | **日期**：2026-08-07

---

## 0. 执行前置（AI 必须遵守）

本规范自身也遵守其 §3.1“标准骨架”要求：执行前置放在文档开头，每条硬性要求均对应可验证产物。凡依据本规范编写或修订业务系统分析类 SOP，AI 必须先完成以下检查，未全部通过不得发布或宣告完成：

- [ ] 确定文档定位、读者对象、优先级与上位约束，落在头部信息块（可验证：头部含"定位/读者对象/优先级/上位约束"四项）
- [ ] 通过三条硬边界检查：不写领域概念、不写项目事实、不写实现细节；逐条用"换领域是否仍成立"判定（可验证：附录自查卡 A 组逐项打勾且通过）
- [ ] 每条关键要求配套可验证执行痕迹：门禁检查、模板加载记录、模板字段对照表、V 验证报告中的至少一项（可验证：正文能指出该要求对应哪一种产物；无对应产物的要求禁止写入）
- [ ] 明确独立验证机制：V 验证由未参与生成的独立 agent 执行并记录，禁止生成者自检（可验证：§6 章节存在且写明"禁止自检"）
- [ ] 关键质量点内嵌合格范例（正例+反例对比），且范例用占位符表达、禁止复制真实类名/参数（可验证：附录自查卡 D 组通过）
- [ ] 修订 SOP 按 §5.1“修订五动作”执行，并同步更新头部版本号与修订记录（可验证：修订记录新增一行 + 头部版本号与之一致）

未通过任一项 → 停止生成，先补齐；不得把"未验证"声明为"已通过"。
...
| v3.2 | 2026-08-07 | 自洽性修复：本规范依照其 §3.1/§3.2 要求补充自身"§0 执行前置"（硬指令 + 逐条可验证产物），使元规范本身符合其主张的"执行前置放最前、无痕迹的要求等于没写"原则；同步头部版本号 | AI |
