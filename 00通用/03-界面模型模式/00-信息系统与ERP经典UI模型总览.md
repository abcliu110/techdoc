...
| `14-深度挖掘方向总览.md` | 8 个深度挖掘方向的总入口和索引 |
| `14a-模式形式化规约.md` | 方向1：模式形式化规约与可机器校验 |
| `15-模式组合代数.md` | 方向2：模式组合算子、兼容矩阵、涌现模式与冲突检测 |
| `16-模式与状态机双向推导.md` | 方向3：状态机→模式正向推导、页面→状态机反向推导与缺陷检测 |
| `17-模式可访问性契约.md` | 方向4：25 个模式的 ARIA、键盘、屏幕阅读器和颜色独立契约 |
| `18-模式性能边界量化.md` | 方向5：数据量、网络、渲染和感知性能的量化阈值与降级策略 |
| `19-Agent-Native-UI模式.md` | 方向6：AI 从辅助变主控的下一代界面模式（10 个 Agent-Native 模式） |
| `20-模式实证验证方法学.md` | 方向7：成熟度分级、证据台账、验证 SOP 和反例库 |
| `21-行业不变量映射.md` | 方向8：财务/库存/权限/合规行业不变量到 UI 校验规则的映射 |
| `22-行业标杆实践深度拆解.html` | 25 个模式在 SAP/Salesforce/Ant Design/金蝶等标杆系统中的实践案例 |
| `23-行业标杆高保真原型.html` + `prototype-styles.css` | 15 个模式的高保真 UI 原型（纯 HTML/CSS） |
| `24-高保真原型与深度拆解.html` | 15 个模式的高保真原型 + 旁边详细文字拆解（图文并茂） |
| `25-标杆企业UI应用框架.md` | SAP Fiori / Salesforce Lightning / Ant Design Pro / IBM Carbon / Fluent / Material 六大框架的架构分层与组件体系 |
| `26-标杆企业UI高阶页面架构.md` | 页面级架构设计：信息架构层级、页面布局分区、跳转路由模式、上下文保持（不含子组件，面向架构师） |
