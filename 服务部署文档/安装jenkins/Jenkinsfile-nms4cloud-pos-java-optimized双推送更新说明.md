# Jenkinsfile-nms4cloud-pos-java-optimized 双推送功能更新说明
...
### 5. buildAndPushDockerImage() 函数更新
...
#### 5.4 推送时间预估
- Harbor 本地仓库：10 MB/s（本地网络）
- 阿里云镜像仓库：0.5-1 MB/s（公网）
- 显示推送目标数量和预估时间
...
if (params.PUSH_TO_HARBOR) {
    ...
    env.HARBOR_IMAGE_FULL = "${harborImageName}:${DOCKER_IMAGE_TAG}"
}
if (params.PUSH_TO_ALIYUN) {
    env.ALIYUN_IMAGE_FULL = "${aliyunImageName}:${DOCKER_IMAGE_TAG}"
    if (!params.PUSH_TO_HARBOR) {
        env.DOCKER_IMAGE_FULL = "${aliyunImageName}:${DOCKER_IMAGE_TAG}"
    }
}
```

### 6. post 成功处理更新
- 分别显示 Harbor 和阿里云的镜像信息
- 提供两种仓库的 kubectl 使用示例
- 优先显示 Harbor 镜像（本地仓库，速度更快）
