# AI 竞品证据驱动的新产品原型统一工程方法

> 版本：5.2
> 状态：全自动自治统一候选规范 5.2（递归 UI 与状态一致性门禁已加强；项目级回归与工具认证尚未全部执行）
> 适用范围：企业软件、Web 应用、桌面工具、低代码平台、设计器、后台系统及其他以人机交互为核心的新产品  
> 核心目标：把“学习竞品并设计更好的产品”从提示词要求，转化为可追溯、可测试、可比较、可阻断的工程过程。

本文已经完成三套方法的重构式融合，统一纳入以下能力：

- 证据、状态、硬门禁、自治治理、审计和领先性验证内核。
- 约束驱动生成、前端工具链、DOM 检测、评分诊断和收敛策略。
- 规则资产、三层验证、自我质疑和用户任务模拟模型。

本文是唯一生效的放行规范，不再保留或依赖合并前的来源文件。合并前的人工签字、总分直接放行、缩减评审法定人数、验证超时后继续、预算耗尽后降级交付等规则均已废止。

---

## 0. AI 执行入口（规范性）

本节是 AI 执行本文件时的最高优先级控制块。第 1 至 27 章用于解释方法、数据结构、质量要求和变更来源；二者冲突时，以本节和各阶段硬门禁为准。

### 0.0 统一规范的权威分层

本文采用四层架构，但只有一套放行语义：

1. **治理内核**：证据链、阶段状态、硬谓词、失败关闭、审计和自治委员会，决定“能否通过”。
2. **执行引擎**：设计 Token、组件白名单、规则引擎、浏览器检测、可访问性和测试脚本，决定“如何验证”。
3. **认知模型**：三层验证、自我质疑、任务模拟和竞品冲突综合，决定“检查什么、为什么检查”。
4. **诊断评分**：多维评分、趋势和候选方案排序，决定“优先修什么、选哪个候选”，不得决定是否放行。

权威关系固定为：

`法律与安全边界 > 冻结任务与硬谓词 > P0/P1 阻断 > 可复现证据 > 诊断评分 > 评审意见 > 示例实现`

任何综合分、LLM 共识或成本策略都不能覆盖未通过的硬谓词、P0/P1、证据完整性、评审独立性或安全真实性要求。
...
生成前加载冻结设计约束和规则集；每完成一个可见单元立即真实渲染、检测和复评；
综合评分只用于诊断和候选排序，不得用于覆盖硬门禁。
```
...
### 0.4.1 视觉可靠性范围与不可裁剪最低门禁

本文所称“视觉可靠性”不是“截图看起来大致正常”，而是以下六类正确性同时成立：

1. **几何正确性**：区域、组件和文本不存在非预期错位、越界、重叠、裁切、离屏、尺寸跳变和滚动陷阱。
2. **内容韧性**：长中文、不可断英文、长数字、空值、极限条目数、国际化和异步失败不会破坏布局或丢失任务必需信息。
3. **层叠正确性**：Portal、Modal、Drawer、Popover、Tooltip、菜单、`fixed / sticky / absolute` 元素和层叠上下文没有错误覆盖、错误裁切或不可命中。
4. **响应式正确性**：目标视口、断点前后、页面缩放、文本缩放、主题、语言、字体环境和方向变化符合冻结合同。
5. **交互状态正确性**：默认、加载、空、错误、禁用、权限、选中、聚焦、拖拽、保存冲突和恢复状态的视觉、DOM/ARIA 与模型状态一致。
6. **视觉语义正确性**：信息层级、密度、对齐、节奏、主次关系、操作可发现性和危险状态表达支持冻结核心任务，而不是只有像素无越界。

任何被本文或下游规范称为“轻量、快速、超轻量、MVP、探索或低风险”的执行模式，只能减少非核心竞品研究、截图数量、诊断评分和主观评审轮次，不得裁剪以下最低门禁：

- 原型构建和真实运行入口有效；未解释的 `console error / page error / critical resource failure` 为零。
- 冻结核心任务可由真实用户等价输入执行，操作结果可见且 DOM/ARIA、模型状态和业务结果断言一致。
- 关键任务区域不存在意外溢出、裁切、遮挡、离屏、不可命中、不可达和无策略双向滚动。
- 至少覆盖标准工作视口、最窄支持视口、最长内容边界以及所有实际响应式断点的相邻样本。
- P0/P1 级视觉、几何、交互和可访问性 finding 为零；不得以“普通页面允许 P1”“人工目视正常”或综合分较高覆盖失败。

其他视觉、研发或审核规范可以补充工具、角色、项目阈值和执行细节；与本节冲突时，以本节为准。下游规范不得把本文的硬门禁降为建议项，也不得让示例代码、人工目视或 LLM 主观评分替代可复现证据。

### 0.4.2 递归 UI 与状态一致性统一门禁

前端原型、设计器、后台工作台和其他具有嵌套布局或可编辑状态的产品，必须同时通过以下两个不可互相替代的门禁：

1. **递归渲染完整性门禁**：以产品的权威结构/状态模型和真实渲染树为双重权威，递归检查任意深度后代；设计器可使用 Schema 树与 DOM 树，其他技术栈映射为领域模型、组件树、原生视图树或等价结构。父组件通过不代表子组件通过。检查至少覆盖四向边界、逐层裁剪交集、声明滚动轴、滚动终点可达、文本完整性、命中测试、浮层逻辑归属、覆盖率和遍历截断。
2. **状态一致性门禁**：先执行架构与能力探测，再选择验证链路，禁止预设所有产品都有 Schema。对每个冻结的可编辑属性、关键命令和产品声明支持的状态能力，从真实用户等价输入开始验证；产品声明支持持久化、刷新恢复、撤销/重做、预览、发布、同步或导出时，才把对应环节追加到链路并逐层断言。未声明且界面未暴露的能力记为 `NOT_APPLICABLE`，不得因未实现该能力判失败。任一适用链路丢值、错投影、产生假成功提示或仅改变视觉样式，均为 P1；导致错误发布、越权或数据破坏时升级为 P0。

架构探测必须基于源码、运行时对象、网络请求、存储或产品文档证据，按以下互斥优先级选择且记录 `state_architecture`、`evidence_refs` 和 `selected_validation_path`：

| 探测结果 | 选择条件 | 必验链路 |
|---|---|---|
| `SCHEMA` | 存在被编辑、保存或渲染器消费的 Schema/页面定义/元数据模型 | `输入/命令 -> 控件状态 -> 内存状态 -> Schema -> Renderer/生成器 -> 可观察业务结果` |
| `EQUIVALENT_MODEL` | 无 Schema，但存在 Store、ViewModel、领域对象、场景树、组件树或其他权威状态模型 | `输入/命令 -> 控件状态 -> 内存状态 -> 等价权威模型 -> 消费者/重渲染 -> 可观察业务结果` |
| `BACKEND_RESULT` | 客户端无可识别权威模型，能力由请求和服务端结果实现 | `输入/命令 -> 请求 -> 服务端权威结果 -> 重新查询/下游读取 -> 可观察业务结果` |
| `PRESENTATION_ONLY` | 产品只声明静态展示或无状态导航，不暴露编辑、保存或业务写入能力 | 只验证已声明的内容、布局、导航、可访问性和视觉交互；Schema、持久化、撤销等记为 `NOT_APPLICABLE` |
| `UNRESOLVED` | 界面声明状态型能力，但现有证据无法识别真实状态来源或结果消费者 | 对应能力为 `BLOCKED`，补充状态来源与消费证据前不得判 PASS |

优先级用于避免重复归类：只有 Schema 确实参与该能力的写入或消费时才选择 `SCHEMA`；代码中仅出现 `schema` 字样不构成证据。不同能力可以选择不同路径，例如页面布局走 `SCHEMA`、发布动作走 `BACKEND_RESULT`，但每项能力只能有一个主验证路径。探测到 Schema 也不自动要求持久化或撤销；这些仍由界面和产品合同是否声明决定。

执行者必须先建立**能力声明与命令覆盖表**：枚举所有可见或可聚焦的按钮、链接、菜单项、拖放入口、键盘命令、表单提交和公开 API，把每个入口映射到前置条件、权威状态变化、可观察结果、失败/禁用状态和证据。已暴露但未实现的命令必须禁用并明确状态；可操作却无结果、只弹提示、只改样式或与命令覆盖表无映射时为 P1。覆盖表必须报告 `已验证命令数 / 已声明命令数`；小于 100% 且缺口涉及冻结核心任务时为 `FAIL`，其他必需命令证据缺失时为 `BLOCKED`。

执行时必须优先调用环境中已安装且通过当前任务能力探测的 `enterprise-ui-quality-gate` Skill。规范引用 Skill 名称和证据合同，不绑定个人目录、绝对安装路径或某个客户端实现。调用后仍必须把项目级视口、断点、滚动所有者、Portal 归属、状态矩阵和持久化介质写入项目视觉/交互合同；Skill 默认值不得覆盖项目合同。

Skill 不可用、版本不满足本节、快速验证失败或无法输出所需证据时，不得跳过门禁，也不得把“手工看过”写成 PASS。执行者只能：

- 按本节和第 9 章的等价检查清单生成机械、交互和视觉证据，并记录 `skill_fallback: equivalent_manual_execution`；或
- 将对应证据通道标记为 `BLOCKED`，原型不得进入 `ENGINEERING_ACCEPTED`。

统一裁决采用失败关闭：任一证据为 `FAIL` 时总结果为 `FAIL`；无 `FAIL` 但存在缺失、过期、视口/状态不匹配或不可重放证据时为 `BLOCKED`；只有机械、交互、视觉证据全部为 `PASS` 且 P0/P1 为零时才为 `PASS`。单元测试直接调用状态更新函数、源码字符串匹配、静态 DOM 快照和最终截图均不能单独证明状态一致性。

每轮执行还必须建立**已知缺陷对照与漏检分析**。将评审前已知缺陷、用户反馈、历史回归、独立评审和本轮人工观察逐项映射到规则、检查动作和实际 finding。规范或工具未发现其中任一已确认问题时，不得省略，也不得用“未发现问题”推导“没有问题”；必须输出漏检项、复现证据、漏检层级、根因分类、最小补强动作和回归夹具。根因至少区分：`SPEC_GAP`（规范缺项）、`TOOL_GAP`（检查器未实现或能力不足）、`COVERAGE_GAP`（视口、状态、角色、数据或嵌套深度未覆盖）、`EXECUTION_GAP`（规则存在但未执行或证据不合格）、`ORACLE_GAP`（通过标准或预期结果错误）、`EVIDENCE_GAP`（证据不可重放或不足）。若当前无法确定根因，标记 `UNRESOLVED` 并将总结果置为 `BLOCKED`；已确认漏检导致 P0/P1 未被报告时，本轮方法执行为 `FAIL`。

漏检闭环必须落到方法资产：规范缺项补规则与反例，工具缺项补检测器或明确等价检查，覆盖缺项补状态矩阵，执行缺项补清单和强制证据，判定器缺项补可复算 oracle，证据缺项补采集合同。每个补强项必须绑定一个能在修复前失败、修复后通过的回归样例；仅修改文字但不增加可执行检查或回归样例，不得宣称漏检已关闭。
...
`EvidenceVerifier` 是五个投票席位中的第五席，只负责从原始产物独立复算证据，不得发起或裁定其本人主张的基线变更。生成者、ReviewCoordinator、PackagingVerifier、ProcessGovernor 不得与任一投票席位兼任；五个投票席位之间也不得兼任。五席必须对应五个可区分执行身份。
...
- 无法重建五个有效独立席位时，委员会 `canonical_result.result=BLOCKED`、`failure_code=INSUFFICIENT_INDEPENDENCE`、`blocker_id=BLK-INDEPENDENCE-REBUILD-SEATS`；不得进入 `AUTONOMOUS_ACCEPTED`。
...
- 不可恢复终态：`AUTONOMOUS_ACCEPTED`、`FAILED`、`INFEASIBLE`。
- 可恢复暂停态：`BLOCKED`、`BUDGET_EXHAUSTED`、`RECOVERY_REQUIRED`。

`BUDGET_EXHAUSTED` 只终止当前预算窗口，不终止状态机；必须保留最新有效检查点的 `successor_action_id`，且只能通过 `BUDGET_RESUME` 原子事务恢复到 `IN_PROGRESS`。`BLOCKED` 只在 blocker 的退出条件满足后恢复；`RECOVERY_REQUIRED` 只在独立签发恢复报告后恢复。
...
- **预防优先于返工**：能在规格、Token、组件 API、类型或构建期阻断的问题，不留到截图阶段发现。
- **规则必须资产化**：质量要求必须有稳定 ID、检测方式、严重级别、精确例外和版本，不能只存在于提示词。
- **评分不得补偿硬缺陷**：评分用于诊断和排序；硬谓词、P0/P1 和 claim 级证据决定放行。
- **任务模拟不是用户研究**：合成用户和 AI 走查只能发现候选风险，不能生成真实满意度或真实完成率。
...
三层验证不是三套裁决制度，而是统一门禁在不同问题域的组织方式：

| 层 | 回答的问题 | 主要执行机制 | 权威输出 |
|---|---|---|---|
| 规则验证 | 是否满足基础工程、设计系统和可访问性约束 | 构建期检查、规则引擎、DOM/axe 检测 | 谓词 `PASS/FAIL` |
| 竞争验证 | 是否在冻结任务和声明上达到或超过对标对象 | 证据矩阵、同条件基准、对抗评审 | claim 级结论 |
| 任务验证 | 用户等价输入能否走通主、异常和恢复路径 | Playwright/浏览器任务脚本、命中和状态断言 | task 级结论 |

三层全部通过只表示具备进入自治验收的资格，不直接产生最终通过。任一层失败必须通过 `return_stage` 返回最上游根因。

### 3.4 每个可见单元的微循环

不得先生成全部原型再统一视觉检查。每个组件、面板或独立场景按以下顺序执行：

`冻结约束 -> 生成/修改 -> 静态规则预检 -> 真实浏览器渲染 -> 机械检测 -> 独立视觉评审 -> 真实交互 -> 可访问性 -> 修复复测 -> 提交检查点`

只有该单元 P0/P1 为零并生成完整证据后，才可以进入下一个依赖单元。相互独立的单元允许隔离并行，但集成工作台必须重新执行完整微循环。

两个循环都通过后，才能进入竞品优势验证和自治评审委员会验收。

### 3.5 八层 UI 可靠性闭环

第 3.4 节的微循环必须实现为八个职责清晰、证据不可互相替代的层次：

| 层 | 目的 | 权威证据 | 失败返回 |
|---|---|---|---|
| 1. 视觉合同冻结 | 在生成前定义区域锚点、尺寸边界、滚动所有权、内容策略、层叠和状态不变量 | 冻结视觉合同及哈希 | 阶段 5 |
| 2. 约束内生成 | 只使用批准 Token、组件和布局原语组合，不允许截图特调 | 静态规则、AST/CSS lint、组件合同 | 阶段 6 |
| 3. 机械视觉检测 | 通过浏览器测量确定几何、裁切、命中、滚动和稳定帧事实 | 原始矩形、计算样式、hit-test、滚动轨迹 | 阶段 6 |
| 4. 独立视觉理解 | 检查机械算法难以判定的层级、密度、节奏、主次和专业性 | 隔离视觉评审及原始截图引用 | 阶段 5/6 |
| 5. 多轴压力验证 | 在视口、状态、内容、主题、语言、字体和缩放组合下发现条件性失真 | 冻结矩阵及逐点报告 | 阶段 5/6 |
| 6. 状态一致性验证 | 用真实输入验证命令、控件、权威模型、重渲染和业务结果；对产品声明的持久化、恢复、撤销/重做、预览、发布或同步能力追加相应链路 | 命令覆盖表、交互轨迹、逐层状态断言及适用能力的恢复证据 | 阶段 5/6/8 |
| 7. 根因化修复 | 返回最上游责任层，禁止用局部 CSS 或假提示掩盖结构/状态问题 | finding、根因、修复事务和失效集合 | 最小 `return_stage` |
| 8. 独立复验 | 由未修改产物的验证身份重跑受影响矩阵并确认无回归 | 新证据对象、前后 finding 差异和 attestation | 阶段 6/7/8 |

机械检测失败不能被视觉模型的“看起来正常”覆盖；视觉语义失败不能仅靠“DOM 无越界”判定通过；状态一致性不能由底层纯函数单测、源码合同或成功 Toast 替代；生成者自检不能替代独立复验。任一层缺少权威证据时，最高只能保留在其上一成熟度状态。
...
| 7. 视觉质量 | 浏览器原型 | 递归几何报告、状态截图、视觉评审 | 递归 UI P0/P1 清零 |
| 8. 任务验证 | 通过视觉门禁的原型 | 任务测试报告、状态一致性轨迹 | 任务达标且模型、持久化、恢复和预览链一致 |
...
<project-root>/
  prototype-method/
    ...
    design-system/
      tokens.json                     # 项目冻结的设计 Token
      component-contracts.yaml        # 组件状态、内容和可访问性合同
      approved-components.yaml        # 允许生成器使用的组件白名单
    rules/
      registry.yaml                   # 规则目录、版本和适用范围
      consistency/                    # 一致性规则
      usability/                      # 可用性和可访问性规则
      layout/                         # 布局、响应式和内容压力规则
      scalability/                    # 数据规模和扩展性规则
    build-gates/                      # lint、typecheck、规则执行结果
    prototypes/                      # 独立可运行原型
    tests/
      interaction-tests.yaml
      task-simulation.yaml
      state-coverage.yaml
...
| 5 | `interaction/*`、`design-system/*`、`rules/*` | IA、主/异常/恢复流程、状态、视觉合同、生成约束和规则注册表齐全 |
| 6 | `prototypes/*`、`build-gates/*` | 每个范围场景有唯一 ID、可运行入口、真实交互，且构建期约束检查通过 |
| 7 | `reports/mechanical/*`、`reports/visual/*` | 每个原型有视口报告、状态截图和独立评审 |
| 8 | `tests/task-simulation.yaml`、`tests/interaction-tests.yaml`、`tests/state-coverage.yaml`、执行结果 | 每个关键任务有模拟风险、真实操作步骤、状态覆盖、断言和结果证据 |
...
| 5 | 主路径、异常路径、恢复路径和权限状态完整；视觉合同可测；Token、组件合同、内容边界、规则集和精确例外在生成前冻结 |
| 6 | 范围场景均有原型；入口可访问；真实交互改变可见状态和机器状态；Token/组件白名单/规则/类型/构建期谓词全部通过 |
| 7 | 目标视口、显示条件、状态组合和集成工作台均有稳定帧报告；控制台无未解释错误；自动可访问性谓词通过；视觉/几何/a11y P0/P1 为零 |
| 8 | 合成用户走查只产出候选风险且明确标记非用户研究；关键任务通过真实用户等价输入执行；命中目标可证明；指针与无指针路径、DOM/可访问状态/模型状态一致；独立测试生成器的任务差异已关闭；失败用例已修复并复测 |
...
优势命题/claim 状态只能按以下顺序升级或失效：

`candidate -> prototype_supported -> benchmark_verified -> invalidated`

`ENGINEERING_ACCEPTED / COMPETITIVE_ACCEPTED / AUTONOMOUS_ACCEPTED` 仅允许出现在 `run.yaml.status`，不得写入优势命题、claim、阶段或单原型状态。
...
每个关键设计决策、组件合同和集成场景在进入原型实现前，生成者必须提交 `design-challenge`。自我质疑不能签发通过，只用于提前暴露风险；答案必须引用证据或明确标记 `UNKNOWN`。

```yaml
design_challenge:
  challenge_id: CHL-DDR-001
  decision_ref: DDR-001
  questions:
    failure_modes:
      answer: 三列布局在最小工作台宽度可能压缩名称
      evidence_refs: [VC-001, EVD-0012]
    novice_comprehension:
      answer: 图标下保留中文名称，说明通过 tooltip 渐进披露
      evidence_refs: [TASK-NOVICE-001]
    scale_100x:
      answer: 组件数量增加时通过搜索、分组虚拟化和最近使用控制扫描成本
      evidence_refs: [ARCH-003]
    competitor_advantage:
      answer: 同宽度下比基线多展示一列，同时保留名称和键盘路径
      evidence_refs: [BM-TOOLBOX-001]
    recovery_and_data_loss:
      answer: 拖拽失败不修改模型；成功投放可撤销
      evidence_refs: [FLOW-DND-002]
  unknowns: []
  return_stage_on_failure: 5
```

五个必答问题域固定为：失败场景、首次用户理解、规模增长、竞品优势及代价、错误恢复与数据损失。出现“以后再考虑”、无证据优势或核心 `UNKNOWN` 时，阶段 5 失败，不得把风险转移给原型实现者。
...
原型生成器不是自由绘图器。阶段 5 必须冻结项目专用的设计系统、组件合同、内容压力和规则集，阶段 6 生成时只能在这些约束内组合。

### 8.0.1 设计 Token

`design-system/tokens.json` 至少声明颜色语义、字体、行高、间距、圆角、阴影、层级、动效、断点和密度模式。Token 数值必须由产品领域和设备合同推导，不能把 `8px`、`12 栅格`或某套框架默认值写成所有产品的通用真理。

生成代码的 Token 使用率必须为 100%；确需例外时，先创建带范围、理由、到期条件和替代方案的冻结例外。发现视觉失败后新增例外，按第 9.2.3 节执行失效和重审。

### 8.0.2 组件白名单与组件合同

生成器只能使用 `approved-components.yaml` 中经过视觉、交互和可访问性验证的组件。每个组件合同至少包含：

- 稳定组件 ID、版本和适用任务。
- 属性类型、必填项、默认值和不变量。
- 默认、悬停、聚焦、激活、选中、禁用、加载、空、错误和权限状态。
- 文本、列表、图片和嵌套内容的典型与极限边界。
- 键盘模型、焦点规则、ARIA 语义和状态公告。
- 允许的容器、最小/最大尺寸、滚动所有权和响应式行为。
- 自动测试抓手、真实拖拽抓手和稳定 `data-testid`。

缺少合同的组件不能进入正式原型；第三方组件必须经过适配合同和独立验证，不能因厂商知名而自动信任。

### 8.0.3 内容压力合同

每个可见区域必须声明 `empty / typical / boundary / adversarial` 四类内容。至少覆盖长中文、长英文不可断词、数字、国际化、空数据、极限条目数、权限差异和异步失败。省略边界不能用 `ellipsis` 或 `overflow: hidden` 静默掩盖；截断必须满足任务可恢复、完整内容可访问和视觉合同允许。

### 8.0.4 规则即资产

所有跨项目质量规则使用统一 schema，不依赖提示词记忆：

```yaml
rule:
  rule_id: UI-LAYOUT-001
  version: "1.0"
  name: 非预期横向溢出
  category: layout
  severity: P1
  scope: [workspace, panel, component]
  detection_method: runtime-dom
  predicate: scrollWidth <= clientWidth + tolerance
  threshold: {tolerance_css_px: 1}
  exceptions_policy_ref: GEO-POLICY-001
  evidence_required: [raw-rectangles, computed-style, screenshot]
  return_stage: 6
  exit_condition: 所有冻结矩阵点谓词通过
```

规则必须具备稳定 ID、版本、严重级别、范围、确定性谓词、阈值、检测方式、证据、返回阶段和退出条件。`warning/info` 可以用于诊断，但所有映射为 P0/P1 的规则都是阻断项。
...
| 约束 | 推荐机制 | 失败语义 |
|---|---|---|
| Token 和任意值 | ESLint/stylelint/AST 规则 | `FAIL + TOKEN_VIOLATION` |
| 组件白名单 | 模块导出边界和依赖扫描 | `FAIL + UNAPPROVED_COMPONENT` |
| 属性与内容边界 | TypeScript/schema validator | `FAIL + CONTRACT_MISSING` |
| 语义和测试抓手 | 静态可访问性与唯一 ID 检查 | `FAIL + TESTABILITY_VIOLATION` |
| 禁止的主布局方式 | AST/CSS 规则和项目例外 | `FAIL + LAYOUT_POLICY_VIOLATION` |

具体技术栈可替换，但不能退化为“已在 Prompt 中说明”。构建期检查通过只允许原型进入 `GENERATED`，不能替代真实浏览器门禁。

### 8.0.6 布局生成策略与禁止的视觉伪修复

生成器在写入布局代码前，必须为每个可见区域声明：`layout_primitive / containing_block / width_policy / height_policy / min_max / wrap_policy / scroll_owner / overlay_owner / anchor_refs / content_pressure_policy / responsive_transition`。声明缺失时不得开始生成该区域。

优先使用能够表达真实约束的 Grid、Flex、容器查询、内容驱动尺寸、`minmax()`、`min-width: 0`、项目布局组件和设计 Token。只有视觉合同明确要求且已有边界测试时，普通业务内容才可使用绝对定位或固定像素尺寸。

以下做法默认禁止；精确例外必须在首次生成前冻结：

- 用固定宽高、魔法数字或截图坐标模拟对齐，而不声明内容和视口边界。
- 用负 `margin`、无来源的 `transform: translate(...)` 或伪元素覆盖修正错位。
- 用 `overflow: hidden`、零高度容器、透明覆盖层或缩小字体隐藏溢出、遮挡和裁切。
- 为解决单个浮层问题不断增加全局 `z-index`，而不修复 Portal 与层叠上下文合同。
- 为当前视口硬编码位置，同时跳过最窄视口、断点、缩放、语言和长文本复验。
- 发现布局失败后只修改提示词措辞、截图或例外，不改变导致失败的结构约束。

状态切换前后的稳定控件尺寸、区域锚点、滚动责任和主操作可达性属于布局不变量。改变这些不变量的修复必须更新视觉合同并触发第 14.2 节基线变更，而不能作为局部样式微调提交。
...
- 页面与区域级锚点图：主轴、对齐线、网格线、边缘和依赖锚点，明确允许的 CSS 像素容差。
- 每个区域的实际包含块、宽高策略、伸缩优先级、最小/最大尺寸和内容增长方向。
- 页面、区域和组件的滚动责任；哪些容器允许水平/垂直滚动，哪些必须自动换行、扩展或分页。
- Portal 宿主、overlay 边界、层叠上下文和焦点生命周期；禁止仅用全局 `z-index` 数值表达层叠合同。
- 文本显示策略：允许换行、允许截断、完整内容访问路径、Tooltip/详情入口和禁止丢失的业务字段。
- 视觉语义不变量：主操作优先级、关键数据可见性、危险状态、密度上限和禁止被次要区域侵占的空间。
- 状态变化不变量：哪些组件和锚点在 hover、focus、selected、loading、error 和 permission 状态下不得改变外部尺寸或位置。
- 视觉压力矩阵轴和最小组合覆盖，至少包含 `viewport x state x content x theme x locale x zoom/font environment`。
...
认证几何检测器不得把“每个元素只与 `parentElement.getBoundingClientRect()` 比较”作为通用溢出算法。验证器必须根据布局机制识别视口、实际包含块、最近裁切祖先、最近滚动祖先、Portal 宿主和声明的 overlay 边界，并同时执行：

1. **四向边界检查**：检测 `left / top / right / bottom` 越界，区分元素自身矩形、视觉溢出、滚动内容和裁切区域。
2. **滚动尺寸检查**：对页面及声明容器比较 `scrollWidth/clientWidth`、`scrollHeight/clientHeight`，验证滚动方向、滚动末端和关键内容可达性；合法滚动不等于自动通过。
3. **文本完整性检查**：测量文本节点和行盒，识别意外截断、逐字换行、不可断词撑开、字体回退和业务必需信息丢失；只测宿主元素矩形不足以通过。
4. **层叠与命中检查**：保留 `absolute / fixed / sticky / portal / transformed` 元素参与分析，通过 stacking context、z-index、裁切祖先及 `elementsFromPoint()` 判断合法覆盖与错误遮挡；不得因定位方式直接排除。
5. **变换与多矩形检查**：考虑 CSS transform、zoom、writing mode、RTL、SVG、伪元素和 `getClientRects()` 多行文本；仅使用一个轴对齐 bounding box 的结论必须标明限制。
6. **可见性检查**：排除未渲染节点，同时识别 `opacity: 0` 覆盖层、`visibility`、`content-visibility`、零面积交互目标和屏幕外但仍拦截事件的节点。
7. **状态差分检查**：比较冻结锚点、区域尺寸、滚动位置、焦点和命中目标在状态切换前后的差异；不能只检查最终单帧。

重叠检测不得对所有 DOM 元素执行无边界的 O(n²) 两两扫描。应先按视觉合同的区域、层叠上下文、空间索引、相邻几何和关键 ROI 缩小候选集，再对候选对计算交集率、遮挡率和命中影响。性能优化不能删除 Portal、浮层和关键操作区域。

Token 合规必须从源 AST、CSSOM 声明、构建产物映射或项目 Token 解析记录判断；`getComputedStyle()` 通常已把 CSS 变量解析为 `rgb/px`，不得通过在 computed style 中搜索 `var(--token)` 判断是否使用 Token。

状态覆盖验证必须实际执行冻结触发步骤，并断言触发前后可见结果、DOM/ARIA、模型状态、错误码和恢复动作；页面上存在某个 `[data-error]` 节点不能证明错误路径已触发或可以恢复。
...
| 区域锚点漂移 | 冻结锚点在同一视口/内容合同下偏移超过合同容差，或对齐线关系改变 | P1 |
| 状态外部尺寸漂移 | 合同声明稳定的组件/区域在状态切换后外部宽高、基线或相邻布局发生变化 | P1 |
| 滚动责任错误 | 未声明容器出现滚动，或声明滚动容器无法访问全部任务必需内容 | P1；阻断核心任务为 P0 |
| 层叠上下文错误 | overlay 被非合同祖先裁切、关键控件被错误层叠覆盖或透明层拦截命中 | P1；阻断核心入口为 P0 |
| 内容语义丢失 | 截断、换行或响应式折叠使任务必需字段无法访问或无法区分 | P1；导致错误操作为 P0 |
...
项目根据目标用户确定矩阵。完整覆盖空间定义为：

`viewport x responsive-boundary x required-state-combination x content-pressure x theme x locale x browser-font-environment x zoom/motion/forced-colors`

不能对完整笛卡尔积逐点执行时，必须使用冻结的 pairwise/风险组合算法，同时把每个 overlay、核心任务、最窄视口、最长内容、错误/权限状态、200% 文字缩放和所有实际断点列为强制组合，不能被 pairwise 抽样删除。企业 PC Web 以下是硬门禁，不是建议项：
...
每个矩阵点都执行 body/container overflow、锚点与对齐线、面板最小宽度、画布最小可用面积、层叠命中、滚动末端、主操作可达、焦点可见和 a11y 检查。内容规模压力按项目合同递增，而不是只放一份理想数据；复杂设计器至少覆盖 1/3/5/10 个同时打开的工具或面板以及空/典型/极限字段数量。

每轮生成后先运行最小高风险矩阵：标准工作视口、最窄视口、最长内容、关键 overlay 打开、一个错误/权限态和 200% 文字缩放。页面或区域完成后再运行完整冻结矩阵。这样可以尽早发现错位，同时不允许用快速循环替代最终覆盖。
...
- 是否在同任务上优于或不低于基准产品。
- 是否出现机械检测无法表达的视觉错位，例如对齐线在不同区域断裂、表格/表单基线不连贯、密度分布失衡、主次关系反转或局部拥挤与大片无效留白并存。
- 内容增长、状态变化、浮层打开和最窄视口下，用户的注意顺序、任务上下文和视觉连续性是否仍成立。

视觉评审必须同时查看主视口与高风险矩阵点的并排证据，不能只审一张最佳截图。评审发现问题时必须标记问题属于信息架构、布局合同、组件合同、内容策略、层叠、状态表达或局部样式中的哪一层；无法定位时先返回证据不足，不直接建议 CSS 数值。
...
评分体系用于让候选方案可比较、让迭代趋势可见，但不具有放行权。项目在首次生成前冻结维度、指标、量纲归一化方法和权重；权重只用于候选排序，不能改变硬谓词。

| 维度 | 建议诊断信号 | 对应硬谓词 |
|---|---|---|
| 视觉正确性 | 几何缺陷密度、状态尺寸漂移 | 几何/稳定帧 P0/P1 为零 |
| 响应式与内容压力 | 冻结矩阵通过率 | 所有必测矩阵点通过 |
| 视觉层级 | 独立视觉 rubric 的保守聚合 | VisualCritic 无未关闭 P0/P1 |
| 信息架构与任务效率 | 路径长度、回退次数、上下文切换 | 冻结核心任务全部可达并达阈值 |
| 设计系统一致性 | Token、组件合同和规则符合率 | 构建期阻断谓词全部通过 |
| 可访问性 | axe/语义/键盘/焦点问题密度 | 自动 a11y P0/P1 为零且无指针路径通过 |
| 竞争优势 | claim 达成率和实际效果量 | 每条领先 claim 独立满足阶段 9 门禁 |

诊断分固定使用 `0..100`，并同时保留原始测量值。总分可以计算，但只允许：
...
禁止行为：用总分补偿 P0/P1、把不同项目的分数直接比较、运行后调整权重使当前候选过线、把 LLM 主观分当作竞品领先证据、因预算耗尽交付最高分但未通过的版本。

LLM 评分必须使用冻结 rubric、隔离上下文和结构化输出。需要稳定排序时至少三次独立采样取中位数；离散度超过冻结阈值时标记 `LOW_CONFIDENCE`，由异模型复验并取保守结果。即使所有评分模型给出高分，仍必须满足机械门禁、任务门禁和五席自治验收。

```yaml
diagnostic_scorecard:
  scorecard_id: SCORE-PT-025-001
  authoritative_for_release: false
  frozen_rubric_hash: sha256:rubric
  hard_gate_result: PASS
  dimensions:
    - id: visual_correctness
      raw_metrics_ref: GEO-PT-025
      normalized_score: 100
      confidence: HIGH
  weighted_total: 91.4
  allowed_use: candidate_ranking
  canonical_result_ref: GATE-STAGE-7
```

评分公式、表格权重和示例计算必须由确定性测试验证一致；任何不一致使评分资产无效，但不得影响硬门禁原始事实。
...
1. **合成任务走查**：多个隔离角色检查可发现性、认知负担、异常、恢复和专家效率，只生成候选 finding。
2. **真实用户等价执行**：浏览器通过真实 click、pointer、keyboard、input 和 drag 操作执行冻结任务，保存命中、事件、状态和结果证据。

合成任务走查必须记录 `simulation_only: true`、角色假设和未知项；不得输出“用户满意度 4/5”“真实完成率 95%”等伪用户数据。没有真实用户参与时，最终报告保持 `real_user_validation: NOT_PERFORMED`。

每个任务至少包含角色、目标、前置条件、数据集、主路径、异常路径、恢复路径、成功断言、失败断言和基准指标。“高频任务不超过三步”只能是项目冻结指标，不能作为所有复杂企业任务的通用常数。

### 独立测试清单双源校验

规格生成者形成 `tasks/tasks.yaml` 后，独立 `TestDeriver` 仅依据冻结章程、竞品特征矩阵和用户目标重新推导验收任务，不读取设计者的任务拆分。`CoverageReconciler` 比较两份清单：

- 交集任务进入正式验收集。
- 仅规格侧存在的任务必须证明其来源，否则删除或降为探索性。
- 仅测试侧存在的任务默认作为缺口，返回阶段 2/5 补充或给出有证据的排除理由。
- 任何核心任务争议未关闭时，阶段 8 为 `FAIL + TASK_COVERAGE_DISPUTE`。

任务执行通过必须同时满足：可定位入口、真实输入命中目标、可见状态与模型状态一致、业务结果断言成立、错误可恢复、无未解释控制台/网络错误。AI 文字走查或 DOM 中存在按钮不能替代这些证据。
...
| 跨区域对齐线断裂、主次关系反转 | 页面布局合同/信息架构 | 给局部元素增加偏移量 |
| 容器伸缩、最小宽度或滚动责任错误 | 页面/区域布局合同 | 用固定宽度或 `overflow: hidden` 掩盖 |
| 组件内部错位、状态尺寸跳变 | 组件合同/组件实现 | 在页面级覆盖组件 CSS |
| 长文本、极值内容导致溢出或语义丢失 | 内容压力合同/内容策略 | 无条件添加 `ellipsis`、缩小字体或隐藏内容 |
| Modal/Popover/Menu 被裁切或错误遮挡 | Portal/层叠上下文合同 | 继续增加全局 `z-index` |
| 响应式断点或缩放下布局崩溃 | 响应式视觉合同 | 只修当前截图尺寸 |
| 机械无越界但层级、密度或节奏失衡 | 信息架构/视觉设计 | 修改检测阈值或声明为主观问题 |
...
### 13.0.1 视觉 finding 根因路由

视觉 finding 必须记录 `symptom / affected_task / reproduction_matrix_point / responsible_contract / root_cause / prohibited_patch / return_stage / exit_condition`。根因按以下优先级判定：

`任务与信息结构 -> 页面区域与布局合同 -> 组件合同 -> 内容与状态策略 -> 层叠/滚动合同 -> 局部样式实现`

只有上游合同均有证据满足时，才允许把问题归类为局部样式实现。以下情况一律视为截图特调并拒绝提交：

- 修复只在一个视口或一条文本上成立，其他冻结矩阵点未复验。
- 修复引入新的魔法数字、负 margin、无合同 transform、全局 z-index 或内容裁切。
- finding 消失但主操作可达性、完整内容访问、焦点路径或视觉层级变差。
- 只更新截图、基线或例外，而没有改变导致失败的实现或合同。

修复提交前后必须计算受影响矩阵点和 finding 集合差异。关闭一个 P0/P1 同时新增同级或更高级 finding 时，按第 13.2.1 节拒绝事务；不能以“原问题已修复”局部放行。
...
每轮必须保存冻结输入哈希、产物哈希、finding 集合、硬门禁结果、诊断分和修复动作。收敛判断以硬 finding 集合为主，诊断分为辅：

| 信号 | 判定 | 确定性处理 |
|---|---|---|
| 同一 finding 修复后两轮内复现 | 震荡 | 回退到复现前检查点，执行根因分析并返回更上游阶段 |
| 修复后新增 P0/P1 多于关闭数量 | 负收益修复 | 拒绝当前事务，不提交权威产物 |
| 连续两轮硬 finding 集合无严格缩小 | 停滞 | 切换实现策略；禁止只改提示词措辞后重跑 |
| 诊断分下降但硬 finding 减少 | 继续但复核 | 不按分数回退，先检查评分噪声和取舍记录 |
| 诊断分上升但硬 finding 未减少 | 假进展 | 不得升级状态或宣称收敛 |

策略切换可以更换布局、组件组合、信息架构或实现方式，但不得减少冻结任务、降低阈值、删除失败样本或扩大例外。需要修改 DesignSpec 时返回阶段 5，并使阶段 6 至 10 失效重跑。

预算、时间和模型调用次数属于资源约束，不能改变质量定义。耗尽时提交最后一个完整事务并进入 `BUDGET_EXHAUSTED/BLOCKED` 或 `FAILED`；严禁“交付最高分版本”、跳过视觉评审、缩减状态矩阵或降低评分/门禁阈值。
...
规范中的代码片段和目录旁现有脚本默认都是 `REFERENCE`，不能因为文件存在或能启动就成为权威验证器。每个参与门禁的工具必须登记：

```yaml
tool_certification:
  tool_id: visual-mechanical-verifier
  artifact_path: scripts/validate-prototype.js
  artifact_hash: sha256:tool-binary
  runtime_hash: sha256:node-playwright-browser
  status: REFERENCE
  supported_predicates: []
  certification_tests:
    positive_fixture: NOT_RUN
    negative_fixture_matrix: NOT_RUN
    timeout_fail_closed: NOT_RUN
    malformed_output_fail_closed: NOT_RUN
    replay_same_result: NOT_RUN
    tamper_detection: NOT_RUN
    environment_drift_invalidation: NOT_RUN
  certified_at: null
  attested_by: null
```

`status` 只能为 `REFERENCE / CANDIDATE / CERTIFIED / REVOKED`。只有 `CERTIFIED` 工具可以为门禁谓词提供权威证据；工具代码、依赖锁、浏览器、配置或运行时发生变化后自动转为 `CANDIDATE` 并重新认证。

视觉机械验证器的认证夹具不能只有一个“正常页”和一个“溢出页”。最低夹具矩阵必须包含：

| 类别 | 阳性或阴性夹具要求 |
|---|---|
| 基础几何 | 正常对齐通过；左/上/右/下越界分别失败 |
| 文本与内容 | 合法换行通过；意外截断、不可断词撑开、长数字和业务字段丢失失败 |
| 容器与滚动 | 合法声明滚动通过；未声明横向滚动、滚动末端不可达和双向滚动陷阱失败 |
| 层叠 | 合法 Modal/Tooltip 覆盖通过；Portal 裁切、sticky/fixed 遮挡、透明层拦截和错误 z-index 失败 |
| 变换与方向 | 合法 transform/RTL/vertical writing 通过；变换后离屏、命中错位和错误 bounding-box 解释失败 |
| 状态变化 | 稳定状态切换通过；选中、聚焦、加载和错误态导致外部尺寸或锚点漂移失败 |
| 响应式与环境 | 合法断点、缩放和字体回退通过；最窄视口、断点相邻点、200% 文本缩放和字体变化崩溃失败 |
| 工具真实性 | 超时、页面崩溃、损坏输出、空报告、篡改证据、浏览器/依赖漂移必须失败关闭或撤销认证 |

认证报告必须为每个夹具保存预期、实际、原始证据哈希和误报/漏报分类。任何必需阴性夹具未被检出、阳性夹具无精确例外仍失败，或同一输入重放结果不一致，工具不得进入 `CERTIFIED`。

合并时附带的参考脚本与配置因缺少主验证器、完整运行输入、依赖锁和工具认证报告，已从本目录移除。本文只定义工具合同、认证条件和门禁语义，不附带可执行验证器；具体项目必须提供经第 14.4 节认证的工具，不能把本文中的示例路径视为现存资产或门禁已执行的证据。
...
- 仅凭截图判定复杂产品整体体验领先。
- 生产环境中的组织变革、培训和运维成本。
- 真实后端接口是否符合业务语义、权限和租户边界是否安全、并发与数据一致性是否正确。
- 金额、整数、内存、调用栈、缓冲区、数据库字段或 AI 上下文 Token 等非 UI 类型的“溢出”。
- 仅用 Mock/fixture 得到的 UI 状态是否等价于真实生产数据、真实故障和长期运行行为。
- 未执行长期性能、内存、跨浏览器和真实设备测试时的生产稳定性。
...
3. 所有页面通过冻结的 `viewport x state x content x theme x locale x browser/font environment x zoom/motion/forced-colors` 风险矩阵；目标视口、断点 sweep、100/125/200/400% 缩放、强制色、减少动画、最长内容和关键 overlay 组合均有证据。
...
13. 设计 Token、组件白名单、内容压力合同、规则注册表和构建期门禁均已冻结并通过。
14. 合成任务走查未被冒充为真实用户研究；独立测试清单双源差异全部关闭。
15. 七维诊断评分与硬门禁分离，评分公式通过一致性测试，最终接受不依赖总分补偿。
16. 使用的验证脚本均在工具认证清单中处于 `CERTIFIED`，脚本、配置和运行时哈希已进入证据链。
17. 所有关键区域的锚点、包含块、滚动责任、层叠上下文、内容显示策略和状态变化不变量均在视觉合同中冻结，并有机械证据与独立视觉证据双重验证。
18. 每个关闭的 P0/P1 视觉 finding 均记录根因层、禁止的伪修复、受影响矩阵和复验结果；不存在只对单张截图生效的补丁。
19. 视觉机械验证器通过第 14.4 节的多类别阳性/阴性夹具矩阵，没有未解释的必需夹具漏报、误报或重放漂移。
...
| 测试 | 测试输入 | 预期行为 |
|---|---|---|
| EXE-001 空项目启动 | 只有项目根目录 | 探测能力，创建 `run.yaml`（初始向量：`run.status=IN_PROGRESS`、`active_stage=null`、阶段 0=`READY`、阶段 1 至 10=`NOT_STARTED`）；若随后立即进入首轮调度，再将阶段 0 原子转为 `IN_PROGRESS`，不直接生成原型 |
...
| EXE-011 总分掩盖缺陷 | 诊断总分 95 但存在一个 P1 | 阶段失败；总分只用于排序，不改变 P1 |
| EXE-012 合成用户冒充真实用户 | AI 走查输出满意度和完成率 | 拒绝指标，标记 `simulation_only` 和 `real_user_validation: NOT_PERFORMED` |
| EXE-013 构建约束缺失 | 原型可运行但用了未批准组件和任意样式值 | 阶段 6 失败，返回补合同或改用白名单组件 |
| EXE-014 视觉检测超时 | 页面未达到稳定帧屏障 | 当前矩阵点 `FAIL`，不得带低置信度继续放行 |
| EXE-015 评审席位不可用 | 五席中一席持续失败 | 委员会结果为 `BLOCKED`，`failure_code=INSUFFICIENT_INDEPENDENCE`；不得缩减为剩余席位全票通过 |
| EXE-016 预算耗尽 | 当前最高分版本仍有 P1 | 进入预算终止状态并保留检查点，不得降级交付 |
| EXE-017 评分公式漂移 | 表格权重与计算实现不一致 | 评分资产失效；硬门禁继续以原始事实判定 |
| EXE-018 独立测试发现漏项 | TestDeriver 有核心任务但规格任务表没有 | 阶段 8 失败并返回上游关闭覆盖争议 |
| EXE-019 截图正常但左上越界 | 元素从视口左侧或顶部离屏，页面无横向滚动条 | 几何门禁失败；不能只检查 right/bottom 或 `scrollWidth` |
| EXE-020 Portal/浮层遮挡 | 菜单通过 Portal 渲染但被 sticky 工具栏遮挡，或透明层拦截点击 | 层叠/命中门禁失败；不得因 absolute/fixed/portal 直接排除 |
| EXE-021 合法滚动与错误滚动 | 一个声明的表格横向滚动和一个未声明的整页横向滚动 | 前者按合同通过；后者失败，证明检测器能区分滚动责任 |
| EXE-022 状态尺寸跳变 | 按钮 loading 后宽度增长并推动相邻主操作 | 机械门禁失败并返回组件合同；不得只截最终稳定截图 |
| EXE-023 长文本伪修复 | 用 ellipsis 隐藏任务必需业务 ID 且无全文访问路径 | 内容语义门禁失败；返回内容/组件合同，不得作为视觉优化通过 |
| EXE-024 轻量流程请求 | 低风险页面要求跳过几何和最窄视口检查 | 拒绝裁剪第 0.4.1 节最低门禁；只能减少非核心评审与证据数量 |
| EXE-025 属性假编辑 | 属性面板按钮变色并弹出成功提示，但没有写入产品权威模型，重渲染后恢复旧值 | 状态一致性门禁失败并返回阶段 6；不得以 Toast、控件样式或底层更新函数单测判定通过 |
| EXE-026 声明能力链断裂 | 产品声明支持持久化、恢复、撤销/重做或预览，但属性只写入内存，刷新后丢失，或不同能力消费了不同状态版本 | 阶段 8 失败；对已声明能力保存控件值、权威模型、快照、恢复和下游消费逐层证据，返回最早断裂层修复；未声明且未暴露的能力不强制实现 |
| EXE-027 可见命令无结果 | 页面存在可点击按钮、菜单项或键盘命令，但点击后无权威状态变化和可观察结果，也未禁用 | 状态一致性门禁失败；命令覆盖表必须列出入口、预期结果和证据，不得用按钮存在或点击无报错判定通过 |
| EXE-028 已知问题未被规范发现 | 独立评审已复现一个 P1，但规范执行报告为 PASS 或未列出该问题 | 方法执行改判 FAIL；生成漏检清单并归类为规范、工具、覆盖、执行、判定器或证据缺口，补充规则、检查动作和修复前失败的回归夹具后重跑 |
| EXE-029 无 Schema 的展示产品 | 静态信息页没有 Schema、保存和撤销能力 | 选择 `PRESENTATION_ONLY`；Schema、持久化和撤销记为 `NOT_APPLICABLE`，只验证页面实际声明的能力，不得因无 Schema 判失败 |
| EXE-030 混合状态架构 | 设计器布局由 Schema 驱动，发布由服务端 API 完成 | 布局选择 `SCHEMA` 链，发布选择 `BACKEND_RESULT` 链；分别提供证据，不得强迫发布结果写回页面 Schema |
| EXE-031 状态来源无法识别 | 页面提供保存按钮并报告成功，但无法找到 Schema、等价模型、有效请求或可重读结果 | 保存能力为 `BLOCKED`；不得擅自归类为展示页面或用成功提示判 PASS |
...
method_lint:
  ...
  checks:
    ...
    - {id: ML-017, assertion: 诊断评分不能覆盖硬谓词或P0/P1}
    - {id: ML-018, assertion: 文档不存在生效的人工审批、缩减法定人数或降级交付路径；废弃条款只允许出现在历史和禁止清单}
    - {id: ML-019, assertion: 合成用户输出不能声明真实满意度或真实完成率}
    - {id: ML-020, assertion: 阶段6要求Token组件合同规则集和构建期门禁}
    - {id: ML-021, assertion: 验证超时和不稳定帧均失败关闭}
    - {id: ML-022, assertion: 权威验证器必须处于CERTIFIED且哈希进入证据链}
    - {id: ML-023, assertion: 未定义状态值不得出现于run stage prototype claim或canonical_result任何状态域}
    - {id: ML-024, assertion: 可恢复暂停态不得出现在不可恢复终态集合且必须有唯一恢复事务}
    - {id: ML-025, assertion: 所有执行模式均保留第0.4.1节视觉可靠性最低门禁}
    - {id: ML-026, assertion: 几何检测合同覆盖四向边界实际包含块滚动层叠命中变换和状态差分}
    - {id: ML-027, assertion: Token合规不得通过computed style中搜索var引用判定}
    - {id: ML-028, assertion: 状态覆盖必须真实触发并断言DOM ARIA模型业务结果与恢复路径}
    - {id: ML-029, assertion: 截断只有在视觉合同允许完整内容可访问且任务信息不丢失时才可通过}
    - {id: ML-030, assertion: 候选规范升级为已验证执行基线必须有工具认证多类型项目回归和CI失败关闭证据}
    - {id: ML-031, assertion: 前端原型必须优先探测并调用enterprise-ui-quality-gate且不可用时失败关闭或执行等价证据清单}
    - {id: ML-032, assertion: 状态一致性覆盖输入命令控件内存权威模型重渲染与业务结果且按产品声明追加持久化恢复撤销预览发布同步链}
    - {id: ML-033, assertion: 纯函数单测源码字符串静态DOM最终截图和成功提示均不能单独证明状态一致性}
    - {id: ML-034, assertion: 所有可见或可聚焦命令均进入命令覆盖表并具有权威状态变化可观察结果或明确禁用状态}
    - {id: ML-035, assertion: 每轮执行均将已知缺陷和独立评审发现映射到规则与finding且所有漏检都有分类根因补强动作和修复前失败的回归夹具}
    - {id: ML-036, assertion: 状态验证先基于证据在Schema等价模型后端结果纯展示或无法识别路径中分流且未声明能力记为NOT_APPLICABLE}
  result: PASS
```
...
1. 运行第 21 章全部 24 个执行自测。
2. 运行第 21.1 节全部方法 Lint。
3. 使用一个最小样例项目从阶段 0 执行到阶段 10。
4. 测试一次中断恢复、一次门禁失败回退、一次工具缺失降级和一次预算耗尽。
5. 比较新旧方法对同一输入的状态和产物差异。
6. 保存 `method-regression-<version>.yaml`；仅当第 21.4 节全部升级条件成立且 P0/P1 为零后，才可把方法标记为已验证自治执行基线。
7. 对七维评分执行权重、公式、边界值和“高分但有P1”反例测试。
8. 对规则注册表执行 schema、重复 ID、过期例外、未知检测器和失效传播测试。
9. 对工具认证清单中的每个 `CERTIFIED` 验证器执行阳性、阴性、超时、损坏输出和重放测试。
10. 使用普通表单/列表、数据密集工作台、复杂浮层页面和响应式页面四类代表项目执行视觉可靠性回归；每类至少包含一个已知正确基线和多个故障注入版本。
11. 记录检测器对各类视觉故障的真阳性、真阴性、误报、漏报、性能和重放一致性；存在未解释的 P0/P1 漏报时不得认证。
12. 在 CI 或等价非交互执行环境中验证：检测成功时通过，P0/P1、超时、浏览器崩溃、空报告、证据损坏或环境漂移时失败关闭。

文档静态检查通过不等于最小样例项目已完成；二者必须分别报告。

## 21.4 从候选规范升级为已验证执行基线

修改版本号、完成静态评审或补充更多规则，都不能单独把本文从“候选规范”升级为“已验证执行基线”。升级必须由独立 `BaselinePromotionVerifier` 从不可变证据中确定性派生，并同时满足：

1. 第 21 章全部执行自测和第 21.1 节全部方法 Lint 通过。
2. 至少一个最小样例项目完整执行阶段 0 至 10，并验证中断恢复、失败回退、能力降级和预算失败关闭。
3. 参与权威门禁的每个工具均为 `CERTIFIED`，完成第 14.4 节全部阳性、阴性、超时、损坏输出、重放、篡改和环境漂移测试。
4. 普通表单/列表、数据密集工作台、复杂浮层页面、响应式页面四类代表项目均完成视觉可靠性回归；每类包含已知正确基线与故障注入。
5. 回归报告给出按故障类别统计的真阳性、真阴性、误报、漏报和性能；不存在未解释的 P0/P1 漏报，误报均有精确可审计原因或修复。
6. CI 或等价非交互环境能在正常样例通过，并在 P0/P1、超时、浏览器崩溃、空报告、证据损坏和运行时漂移时失败关闭。
7. 独立验证器能够从工具认证、项目回归、CI 运行和审计链重算同一结论；证据哈希、方法版本、浏览器和运行时版本全部匹配。

升级产物至少包含 `baseline-promotion-5.1.yaml`、工具认证索引、四类项目回归清单、CI 运行引用和限制声明。任一条件为 `NOT_RUN / UNKNOWN / STALE / FAIL` 时，状态继续保持候选；不得用委员会投票、主观信心或“规则已经足够完整”代替执行证据。
...
至此 Codex 4.0 基线累计完成 15 轮评审。该历史记录不等同于 5.0 合并版已经完成第 21.3 节最小样例项目回归。
...
| 来源 | 吸收内容 | 统一后的权威位置 |
|---|---|---|
| Codex 4.0 | 证据供应链、阶段门禁、状态机、自治五席、视觉真实性、统计基准、审计与恢复 | 第 0、4、5、7、9 至 14、19 至 22 章 |
| GLM | 约束驱动生成、Token/组件限制、DOM 检测、评分诊断、震荡和成本控制、独立测试思路 | 第 8.0、9.6、10.1.1、13.2.1、14.4 节 |
| Fable | 三层验证、规则 schema、自我质疑、状态覆盖和任务模拟 | 第 3.3、6.5、8.0.4、9.4、10.1.1 节 |

### 26.2 改造后吸收

- GLM 七维加权评分改为非权威诊断评分；硬谓词和 P0/P1 仍决定放行。
- GLM 多 LLM 评分稳定性方法仅用于候选排序；最终自治验收仍按五席、独立性和 EvidenceVerifier 规则。
- GLM 的脚本片段改为参考实现；通过第 14.4 节认证后才能成为门禁工具。
- Fable 的用户任务模拟改为候选风险发现；禁止输出伪真实用户完成率和满意度。
- Fable 的规则系统与 GLM 的构建期约束合并为“规则资产 + 前移执行 + 浏览器复验”。

### 26.3 明确废弃

以下合并前条款不得进入实现或作为替代路径：

1. 人工签字、人工终审或超过重试次数后升级人员决定。
2. 综合分达到阈值直接放行，或允许高分补偿硬缺陷。
3. LLM 席位失败后将法定人数从三席/五席缩减为剩余席位。
4. 预算、时间或调用次数耗尽后交付最高分但未通过的版本。
5. 布局未稳定或检测超时后继续采集并以低置信度放行。
6. 仅凭 DOM selector 存在认定业务功能已经实现或超过竞品。
7. 把不同 `z-index`、父子关系或滚动容器一概排除为合法重叠。
8. 将固定 12 栅格、固定断点、固定三步路径等示例数值设为跨产品通用门禁。

### 26.4 5.0 状态声明

本文完成的是三篇方法的规范性重构与静态合并，不表示：

- 第 21.3 节最小样例项目回归已经执行。
- 本目录参考脚本已经通过工具认证。
- 真实用户研究、市场验证或生产验证已经执行。
- 任一具体产品已经获得 `AUTONOMOUS_ACCEPTED`。

独立多角色静态评审已经完成；在方法回归和工具认证完成前，本文状态仍保持“统一候选规范”，不得宣称为已验证执行基线。

### 26.5 5.0 合并评审记录

本次合并后使用隔离上下文执行规范治理、企业前端设计与规范一致性三类评审，并对修复项进行定向复审：

| 评审角色 | 首轮发现 | 修复 | 定向复审 |
|---|---|---|---|
| 规范治理与状态机架构师 | 1 个 P0、3 个 P1：claim/run 状态混用、预算状态歧义、独立性结果编码不统一、初始状态自测冲突 | 统一五类状态域、暂停/终态、canonical failure code 和初始向量 | 4/4 关闭，0 个 P0/P1 |
| 企业前端设计系统与视觉测试专家 | 0 个 P0/P1 | 无需修改 | 通过 |
| 规范一致性审查员 | 1 个有效 P1：EvidenceVerifier 席位身份歧义；另一个 claim finding 来自修复前快照 | 明确 EvidenceVerifier 是第五席，五席对应五个独立身份 | 关闭，0 个 P0/P1 |

静态评审只证明当前文档未发现剩余的文档级 P0/P1，不替代第 21.3 节方法回归、工具认证、具体项目门禁或真实用户验证。

## 27. 5.2 UI 可靠性加强记录

5.2 在 5.1 视觉可靠性基础上，把递归渲染完整性和状态一致性纳入同一失败关闭门禁，并把 `enterprise-ui-quality-gate` 设为优先执行入口。

### 27.1 本次加强内容

- 保留第 0.4.1 节几何、内容、层叠、响应式、交互状态和视觉语义六类视觉正确性及不可裁剪最低门禁。
- 在第 0.4.2 节增加权威模型/渲染树双树递归检查、能力声明驱动的状态一致性链、命令覆盖表和 Skill 优先执行/不可用失败关闭规则。
- 在第 3.5 节把七层视觉闭环扩展为八层 UI 可靠性闭环，增加真实输入驱动的状态一致性验证层。
- 在第 8.0.6 和 8.3 节增加布局生成策略、禁止的截图特调、区域锚点、包含块、滚动责任、层叠上下文、内容显示和状态不变量。
- 在第 9 章加强四向边界、实际包含块、滚动容器、Portal、fixed/sticky/absolute、transform、hit-test、文本完整性和状态差分检测。
- 在第 13 章建立视觉 finding 根因路由，防止所有错位、溢出和层级问题被降格为局部 CSS 调整。
- 在第 14.4 节增加多类别阳性/阴性认证夹具，明确简单脚本片段不能成为权威检测器。
- 在第 21 章增加属性假编辑、声明能力链断裂和可见命令无结果反例，以及 Skill 调用、状态一致性和命令覆盖方法 Lint。
- 增加已知缺陷对照与漏检闭环：规范未命中已确认问题时必须失败关闭，区分规范、工具、覆盖、执行、判定器和证据缺口，并用回归夹具证明补强有效。

### 27.2 当前状态

本次修改完成的是规范层 UI 可靠性加强，不表示：

- 视觉机械验证器已经实现或取得 `CERTIFIED` 状态。
- 第 14.4 节多类别夹具已经运行。
- 四类代表项目的视觉可靠性回归已经完成。
- CI 失败关闭和 `BaselinePromotionVerifier` 已经运行。
- AI 原型的全部视觉错位、溢出或其他视觉缺陷已经在真实项目中被证明解决。

因此 5.2 仍保持“统一候选规范”。只有第 21.4 节全部条件有可复算证据时，才能发布新的状态声明并升级为“已验证执行基线”；在此之前只能声明“解决方案和验收标准已定义”，不能声明“工程能力已经跑通”。
