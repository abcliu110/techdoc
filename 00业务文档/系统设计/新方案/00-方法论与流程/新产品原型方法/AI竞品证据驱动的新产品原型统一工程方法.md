...
本节是 AI 执行本文件时的最高优先级控制块。第 1 至 26 章用于解释方法、数据结构、质量要求和变更来源；二者冲突时，以本节和各阶段硬门禁为准。
...
`status` 只能为 `REFERENCE / CANDIDATE / CERTIFIED / REVOKED`。只有 `CERTIFIED` 工具可以为门禁谓词提供权威证据；工具代码、依赖锁、浏览器、配置或运行时发生变化后自动转为 `CANDIDATE` 并重新认证。认证测试必须证明工具既能让正确夹具通过，也能让溢出、覆盖、不可达、超时、损坏输出和篡改证据失败。

本目录在合并时存在 `scripts/orchestrator.js`、`scripts/scoring.js`、`scripts/axe-audit.js` 等参考资产，但 `package.json` 引用的 `scripts/validate-prototype.js` 不存在，且未发现这些工具的完整认证报告。因此它们当前一律为 `REFERENCE`，不能作为本文 5.0 已代码完备或门禁已执行的证据。
