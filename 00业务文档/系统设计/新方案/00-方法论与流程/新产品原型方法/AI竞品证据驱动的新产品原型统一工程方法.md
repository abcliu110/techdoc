...
> 版本：5.1  
> 状态：全自动自治统一候选规范 5.1（视觉可靠性条款已加强；项目级回归与工具认证尚未执行）  
> 适用范围：企业软件、Web 应用、桌面工具、低代码平台、设计器、后台系统及其他以人机交互为核心的新产品  
> 核心目标：把“学习竞品并设计更好的产品”从提示词要求，转化为可追溯、可测试、可比较、可阻断的工程过程。

本文由以下三套方法重构式融合而成，来源文件继续保留，不作为并行生效的放行规范：

- `codex-AI竞品证据驱动的新产品原型工程方法.md`：提供证据、状态、硬门禁、自治治理、审计和领先性验证内核。
- `glm-AI原型设计闭环验证方法论.md`：提供约束驱动生成、前端工具链、DOM 检测、评分诊断和收敛策略。
- `fable-AI原型设计与质量验证一体化方法论.md`：提供规则资产、三层验证、自我质疑和用户任务模拟模型。

三篇来源的条款发生冲突时，不按多数、篇幅或更新时间裁决，只执行本文的单一规则。来源中的人工签字、总分直接放行、缩减评审法定人数、验证超时后继续、预算耗尽后降级交付等规则均已废止。
...
第 3.4 节的微循环必须实现为七个职责清晰、证据不可互相替代的层次：
...
| 6. 根因化修复 | 返回最上游责任层，禁止用局部 CSS 掩盖结构问题 | finding、根因、修复事务和失效集合 | 最小 `return_stage` |
| 7. 独立复验 | 由未修改产物的验证身份重跑受影响矩阵并确认无回归 | 新证据对象、前后 finding 差异和 attestation | 阶段 6/7 |

机械检测失败不能被视觉模型的“看起来正常”覆盖；视觉语义失败不能仅靠“DOM 无越界”判定通过；生成者自检不能替代独立复验。任一层缺少权威证据时，最高只能保留在其上一成熟度状态。
...
本目录在合并时存在 `scripts/orchestrator.js`、`scripts/scoring.js`、`scripts/axe-audit.js` 等参考资产，但 `package.json` 引用的 `scripts/validate-prototype.js` 不存在，且未发现这些工具的完整认证报告。因此它们当前一律为 `REFERENCE`，不能作为本文 5.1 已代码完备或门禁已执行的证据。
...
5.1 在 5.0 单一治理、状态机、证据和工具认证框架之上，加强 AI 设计与实现原型时的整体视觉可靠性，目标不局限于横向溢出检测。

### 27.1 本次加强内容

- 在第 0.4.1 节定义几何、内容、层叠、响应式、交互状态和视觉语义六类视觉正确性，并建立任何轻量模式都不得裁剪的最低门禁。
- 在第 3.5 节建立视觉合同、约束生成、机械检测、独立视觉理解、多轴压力、根因修复和独立复验七层闭环。
...
- 在第 19 和 21 章增加视觉可靠性完成条件、执行反例、多类型项目回归、CI 失败关闭和候选基线升级标准。

### 27.2 当前状态

本次修改完成的是规范层视觉可靠性加强，不表示：
...
因此 5.1 仍保持“统一候选规范”。只有第 21.4 节全部条件有可复算证据时，才能发布新的状态声明并升级为“已验证执行基线”；在此之前只能声明“解决方案和验收标准已定义”，不能声明“工程能力已经跑通”。
