...
- ADR 状态行统一使用 `> 状态：accepted`、`> 状态：accepted for Mx`、`> 状态：proposed`、`> 状态：deprecated` 或 `> 状态：superseded`；缺失、未知或无法解析的状态一律按未批准处理。
- `accepted` 或 `accepted for Mx` 的 ADR，才是对应范围内可直接进入代码、DDL、公共 DTO 和阻断性测试的实现事实。
- `proposed` ADR 只提供候选实现。PRD 已确认的业务不变量仍然有效，但候选类名、字段、表结构、枚举、优先级、状态机和算法不得因此成为强制实现。
- 任务需要消费 proposed ADR 的精确设计时，开工前必须将 ADR 终审为 accepted，或形成等价的已批准版本化决策记录；随后同步更新详细设计、任务卡、测试规格和能力追溯矩阵。
- 未通过上述门禁时，可以做不锁定结构的接口探索、契约测试草案和风险验证，不能把候选项计入完成度或声明里程碑已就绪。
...
- 每个任务必须提交测试证据。
- 发现详细设计与 PRD、架构设计、`../../../../工程规范/README.md` 所列现行通用/低代码规范或 accepted ADR 冲突时，停止实现并先统一事实源。
- 涉及单向门的新决策必须新增 ADR。
