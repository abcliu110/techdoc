...
(
    ...
    `mb_amount` Decimal(18, 4) COMMENT '会员卡消费',
    `mb_false_amount` Decimal(18, 4) COMMENT '会员卡赠送消费',
    `revenue` Decimal(18, 4) COMMENT '实收金额',
    `false_revenue` Decimal(18, 4) COMMENT '虚收金额'
)
ENGINE = ReplacingMergeTree(created_time)
PARTITION BY toYYYYMM(report_date)
PRIMARY KEY (mid, sid, year_, month_, day_, name)
ORDER BY (mid, sid, year_, month_, day_, name)
