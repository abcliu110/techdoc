...
当前平台不是完全没有原子能力，而是存在三类问题：

### 2.1.1 目录原子不是执行原子

部分能力已经写入 `business-capabilities.json`，可以被平台查询、展示、编排，但真实运行时仍然调用大脚本中的一段流程逻辑。

典型例子：`miniapp.orderDishByAmount`。

它在目录中是一个 capability，但实际执行仍依赖 `run-miniapp-order-pay-benefits.mjs` 内部流程。这样的问题是：

- 不能单独运行该能力。
- 不能单独判定该能力成功或失败。
- 不能独立输出前置状态、后置状态和失败证据。
- 一个大 stage 失败后，只能从日志里倒查真正失败点。

整改判断：目录原子只能算“能力目录项”，不能算“运行时原子能力”。

### 2.1.2 部分执行原子质量不够

有些逻辑虽然看起来像原子操作，但没有达到可复用能力标准。

以小程序加菜为例，当前做法是：

- 扫描大量 `view/text`。
- 根据 class/text 猜测加菜按钮。
- 连续尝试 `tap / trigger / touch`。
- 最后通过请求或页面文本推断是否成功。

这类能力的问题是：

- 操作对象不够确定，容易点错元素。
- 动作边界不清晰，一次业务动作中混入多种尝试。
- 点击后没有立即验证页面是否还在预期状态。
- 失败分类不够准确，容易把页面丢失、请求未发、请求失败混在一起。
- 不利于多个用例复用。

整改判断：执行原子必须满足“明确前置状态、明确操作对象、明确后置断言、明确失败分类、明确 evidence”的标准。

### 2.1.3 用例没有完全按原子能力编排

当前部分用例仍然是一个大 runner 包含多个业务步骤，例如：打开点餐页、加菜、提交订单、付款都在一个 stage 中。

这样会导致：

- 某一步失败后，后续没有执行的能力也可能被 summary 标记 failed。
- 平台无法沉淀“哪个原子能力不稳定”。
- 不能复用同一个能力去验证不同用例。
- 不能对单个能力做快速回归。

整改判断：用例编排必须从“调用大脚本”升级为“调用能力链”，大脚本只能作为短期 adapter 包装存在。

### 2.1.4 原子能力验收标准

一个能力只有同时满足以下条件，才能算真正的运行时原子能力：

1. 有独立 capabilityId。
2. 有明确输入和输出。
3. 有前置状态检查。
4. 有单操作 10 秒以内的 UI action gate。
5. 有后置业务断言。
6. 有失败分类。
7. 有独立 evidence。
8. 可单独运行或可在 stage 内独立记录状态。
9. 被多个用例复用时不复制判断逻辑。

不满足这些条件的，只能算目录项、脚本片段或临时流程逻辑，不能算稳定原子能力。
