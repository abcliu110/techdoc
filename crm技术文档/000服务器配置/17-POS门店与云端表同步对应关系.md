...
从 Kafka 到门店的自动实时投递链路当前存在断点，不能再把下面的历史路径当作现行实现：

- `nms4cloud-pos4cloud` 原 `KafkaListenerForSync` 曾按 `sid` 通过 Netty 调用门店 `/sync/syncByCanalEvent`，但已在提交 `98c1a0433` 中删除。
- 当前同名监听器位于 `nms4cloud-pos5sync`，只打印收到的消息，没有向门店转发。
- 门店 `nms4cloud-pos3boot` 仍保留 `/sync/syncByCanalEvent` 接口、JMS 消费者和 `IncrementalSyncDataService.handleEvent(...)` 处理逻辑，但当前仓库内没有找到把 Kafka 消息送入该接口的有效调用方。

门店还保留主动增量拉取路径：`/sync/incrementalSyncData` 经 `nms4cloud-pos4cloud` 的 `/canal_event/list` 查询 `nms4cloud-pos5sync` 事件数据，再按 `tbl_name` 处理。该路径上游是否持续完整写入事件数据，仍需结合部署配置和运行态验证。
...
| `pt_auto_order` | `pt_auto_order` | `PtAutoOrder` | 门店业务数据 |
...
| `biz_pos_perm_policy` | `biz_pos_perm_policy` | `BizPosPermPolicy` | POS 操作权限策略 |
| `biz_pos_perm_discount` | `biz_pos_perm_discount` | `BizPosPermDiscount` | POS 折扣权限 |
| `biz_pos_perm_dish_scope` | `biz_pos_perm_dish_scope` | `BizPosPermDishScope` | POS 菜品范围权限 |
| `biz_pos_perm_quota` | `biz_pos_perm_quota` | `BizPosPermQuota` | POS 操作额度权限 |
| `biz_pos_perm_payment` | `biz_pos_perm_payment` | `BizPosPermPayment` | POS 支付权限 |
...
- Git 历史中的 `nms4cloud-pos4cloud/.../KafkaListenerForSync.java`
  - 历史实现曾监听 Kafka topic `nms4cloud-pos5sync`，并按 `canalEvent.sid + "_server"` 通过 Netty 调用门店 `/sync/syncByCanalEvent`。
  - 该文件已在提交 `98c1a0433` 中删除，不能作为当前链路依据。

- 当前 `nms4cloud-pos5sync/.../KafkaListenerForSync.java`
  - 仍监听 Kafka topic `nms4cloud-pos5sync`。
  - `consumeMessage(...)` 只打印消息，没有门店转发逻辑。

- `nms4cloud-pos4cloud/.../CanalEventController.java`
  - `/canal_event/list` 将门店的事件查询代理到 `nms4cloud-pos5sync/platform/canal_event/list`。

- `nms4cloud-pos3boot/.../SyncByCanalEventConsumer.java`
  - 门店 JMS 收到消息后转成 `CanalEventVO`。
...
- environment: source inspection only; production Nacos、Kafka、数据库未查询

文中标记“待运行态验证”的链路和映射不能替代部署配置、消息堆积、数据库记录或真实门店事件的现场证据。
