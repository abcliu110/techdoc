...
CREATE TABLE `pt_dish_area`  (
  ...
  `updated_time` datetime NULL DEFAULT NULL COMMENT '更新时间',
  `deleted` bigint NULL DEFAULT NULL COMMENT '是否删除',
  `not_inherit_common` tinyint NULL DEFAULT 0 COMMENT '不继承通用做法：0-否，1-是',
  `not_multiple_choice` tinyint NULL DEFAULT 0 COMMENT '不可多选：0-否，1-是',
  `not_show_practice` tinyint NULL DEFAULT 0 COMMENT '不自动弹出：0-否，1-是',
  `fixed_cook` tinyint NULL DEFAULT 0 COMMENT '做法不随菜谱变更：0-否，1-是',
  PRIMARY KEY (`pid`, `lid`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 188 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '菜品部位' ROW_FORMAT = Dynamic;
...
CREATE TABLE `pt_dish_flavor`  (
  ...
  `deleted` bigint NULL DEFAULT NULL COMMENT '是否删除',
  PRIMARY KEY (`pid`, `lid`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 16892 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '菜品口味' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Table structure for pt_dish_overtime
-- ----------------------------
...
CREATE TABLE `pt_dish_overtime`  (
  ...
  `sid` bigint NULL DEFAULT NULL COMMENT '门店号',
  `lid` bigint NOT NULL COMMENT '逻辑编号',
  `relate_lid` bigint NULL DEFAULT NULL COMMENT '关联标识',
  `type_` int NULL DEFAULT NULL COMMENT '类型 1菜品 2菜品小类 3菜品大类',
  `side_over_time` int NULL DEFAULT NULL COMMENT '配菜超时时间（分钟）',
  `cook_over_time` int NULL DEFAULT NULL COMMENT '制作超时时间（分钟）',
  `serve_over_time` int NULL DEFAULT NULL COMMENT '传菜超时时间（分钟）',
  ...
  INDEX `idx_mid_sid_relate_lid_type_`(`mid` ASC, `sid` ASC, `relate_lid` ASC, `type_` ASC) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '商品超时设置' ROW_FORMAT = Dynamic;
...
CREATE TABLE `pt_flavor_type`  (
  ...
  `deleted` bigint NULL DEFAULT NULL COMMENT '是否删除',
  PRIMARY KEY (`pid`, `lid`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 701 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '口味类型' ROW_FORMAT = Dynamic;
...
) ENGINE = InnoDB AUTO_INCREMENT = 22195 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '图片压缩记录' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Table structure for pt_label
-- ----------------------------
...
CREATE TABLE `pt_label`  (
  ...
  `type_` int NULL DEFAULT NULL COMMENT '标签类型',
  `hideInWx` tinyint(1) NULL DEFAULT NULL COMMENT '扫码下单隐藏',
  `revision` int NULL DEFAULT NULL COMMENT '乐观锁',
  `created_by` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL COMMENT '创建人',
  `created_time` datetime NULL DEFAULT NULL COMMENT '创建时间',
  `updated_by` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL COMMENT '更新人',
...
) ENGINE = InnoDB AUTO_INCREMENT = 16 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '菜品标签' ROW_FORMAT = Dynamic;
